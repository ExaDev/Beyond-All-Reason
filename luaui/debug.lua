...
function PrintSelection()
	local udTable, n = Engine.Unsynced.GetSelectedUnitsSorted()
...
function PrintCommandQueue(uid)
	local queue = Engine.Shared.GetUnitCommands(uid, -1)
	if queue ~= nil then
		local msg = ""
		local count = 0
...
function PrintAlliedUnits()
	local teamTable = Engine.Shared.GetTeamList(Engine.Unsynced.GetLocalAllyTeamID())
	...
	for atn, atid in ipairs(atTable) do
		...
		for tn, tid in ipairs(tTable) do
			...
			for pn, pid in ipairs(pTable) do
				local pname, active = Engine.Shared.GetPlayerInfo(pid, false)
				if active then
...
function PrintTeamInfo(teamID)
	local num, leader, dead, isAI, side, allyTeam = Engine.Shared.GetTeamInfo(teamID, false)
...
function PrintTeamResources(teamID, type)
	local current, storage, pull, income, expense, share, sent, received = Engine.Shared.GetTeamResources(teamID, type)
	if current ~= nil then
...
function PrintPlayerInfo(playerID)
	local name, active, spectator, team, allyteam, ping, cpuUsage = Engine.Shared.GetPlayerInfo(playerID, false)
	print("   name:     " .. name)
	print("   id:       " .. playerID)
...
function DebugDumpDefs()
	...
	PrintTeamUnits(Engine.Unsynced.GetLocalTeamID())

	print("My Player Info:")
	local myPlayerID = Engine.Unsynced.GetLocalPlayerID()
	PrintPlayerInfo(myPlayerID)

	PrintTeamUnitsCounts(Engine.Unsynced.GetLocalTeamID())
end
