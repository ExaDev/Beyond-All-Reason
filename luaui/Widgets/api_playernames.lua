	...
	if params[1] then
		...
		if playerID then
			...
			if accountID then
				...
				if alias then
					spEcho(BAR.I18N("ui.playernames.setalias", { name = name, accountID = accountID, alias = alias }))
					...
					if history[accountID] and history[accountID].alias then
						spEcho(BAR.I18N("ui.playernames.removealias", { name = name, accountID = accountID, alias = history[accountID].alias }))
						currentNames[playerID] = name
						currentAccounts[accountID] = name
...
function widget:SetConfigData(data)
	...
	if data.gameID and data.gameID == (Game.gameID and Game.gameID or Engine.Shared.GetGameRulesParam("GameID")) then
