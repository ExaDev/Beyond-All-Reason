...
local metaElement = {
	vsx = vsx,
	vsy = vsy,
	vbokeys = { left = 1, bottom = 2, right = 3, top = 4, tl = 5, tr = 6, br = 7, bl = 8, color1r = 9, color1g = 10, color1b = 11, color1a = 12, color2r = 13, color2g = 14, color2b = 15, color2a = 16, uvbottom = 17, uvleft = 18, uvtop = 19, uvright = 20, fronttexture = 21, edge = 22, zdepth = 23, progress = 24, hide = 25, blendmode = 26, globalbg = 27, unused = 28 },
...
function metaElement:ProcessRelativeCoords()
	...
	if not self.left then
		self.left = self.parent.left + padding
	end
	if not self.right then
		self.right = self.parent.right - padding
	end
	if not self.bottom then
		self.bottom = self.parent.bottom + padding
	end
	if not self.top then
		self.top = self.parent.top - padding
	end
end
...
local function newElement(o) -- This table contains the default properties
	...
	if not obj.isroot then
		...
		if parent.layer then
			...
			BAR.Debug.TraceEcho(obj.name .. " parented to " .. obj.parent.name .. " has no layer")
		end
	end
...
function metaElement:UpdateTextPosition(newtext) -- for internal use only!
	...
	if debugmode then
		spEcho(newtext.alignment, newtext.text, newtext.textwidth, newtext.textheight, elementwidth, elementheight)
	end
...
function metaElement:GetElementUnderMouse(mx, my, depth)
	...
	if mx >= self.left and mx <= self.right and my <= self.top and my >= self.bottom then
		hit = true
	end
	--spEcho("result:",hit)
	if hit == false then
...
function metaElement:Destroy(depth)
	...
	if self.textelements and next(self.textelements) then
		self.layer.textChanged = true
	end
...
function metaElement:NewUiElement(o) -- A UiElement
	local obj = newElement(o)
	obj.instanceKeys = Draw.Element(obj.VBO or rectRoundVBO, obj.name, obj.depth, obj.left, obj.bottom, obj.right, obj.top, obj.tl, obj.tr, obj.br, obj.bl, obj.ptl, obj.ptr, obj.pbr, obj.pbl, obj.opacity, obj.color1, obj.color2, obj.bgpadding or 3)
	return obj
end
...
function metaElement:NewButton(o) -- yay this objs shit again!
	...
	obj.instanceKeys = Draw.Button(obj.VBO or rectRoundVBO, obj.name, obj.depth, obj.left, obj.bottom, obj.right, obj.top, obj.tl or 1, obj.tr or 1, obj.br or 1, obj.bl or 1, obj.ptl or 1, obj.ptr or 1, obj.pbr or 1, obj.pbl or 1, obj.opacity or 1, obj.color1, obj.color2, obj.bgpadding or 3)
	return obj
end
...
function metaElement:NewEditBox(obj) end

function metaElement:NewComboBox(obj) end
...
function metaElement:NewSlider(o)
	...
	o.MouseEvents = {
		left = function(obj, mx, my)
			...
			if debugmode then
				spEcho("left clicked", obj.name, mx, wratio, newvalue, newright)
			end
		...
		right = function(obj, mx, my)
			...
			obj:UpdateVBOKeys("right", newright)
			if debugmode then
				spEcho("right clicked", obj.name, mx, my, newright, obj.value)
			end
		end,
	...
	obj.updateValue = function(obj, newvalue, index, tag)
		if debugmode then
			spEcho("updateValue", obj.name, newvalue, index, tag, obj.valuetarget)
		end
		...
		if obj.callbackfunc then
			obj.callbackfunc(obj.name, newvalue, index, oldvalue)
		end
	end

	obj.instanceKeys = Draw.Slider(obj.VBO or rectRoundVBO, obj.name, obj.depth, obj.left, obj.bottom, obj.right, obj.top, obj.steps, obj.min, obj.max)

	if obj.VBO.dirty then
		uploadAllElements(obj.VBO)
	end
...
function metaElement:NewUiUnit(o)
	...
	obj.instanceKeys = Draw.Unit(
		obj.VBO or rectRoundVBO,
		obj.name,
		obj.depth,
		obj.left,
		obj.bottom,
		...
		obj.zoom or 1,
		obj.bordersize,
		0.8, --zoom,  borderSize, borderOpacity
		obj.texture,
		obj.radartexture,
		obj.grouptexture,
		obj.price,
...
function metaElement:NewWindow(o)
	...
	local titlebutton = metaElement:NewButton({
		...
		MouseEvents = {
			drag = function(obj, mx, my, lastmx, lastmy)
	...
	if o.testsliders then
		...
		for i, rando in ipairs({ "one", "two", "three", "four", "five", "six", "seven", "eight", "nine", "ten", "1", "2", "3", "4", "5", "6", "7", "8" }) do
			...
			local newSlider = metaElement:NewSlider({
				...
				defaultvalue = 3,
				valuetarget = nil,
				callbackfunc = function(name, val)
					spEcho(name, val)
				end,
				index = i,
			})
...
local function uiUpdate(mx, my, left, middle, right)
	...
	if left and not lastmouse.left and elementundercursor and elementundercursor.MouseEvents and elementundercursor.MouseEvents.drag then
		draggableelement = elementundercursor
	elseif left == false then
		draggableelement = nil
	end

	if draggableelement and left and lastmouse.left and ((mx ~= lastmouse.mx) or (my ~= lastmouse.my)) then -- drag
	...
	if useTextDisplayList then
		for layername, Layer in pairs(Layers) do
			if Layer.textChanged then
				if Layer.textDisplayList then
					gl.DeleteList(Layer.textDisplayList)
				end
				local textcount = Layer:DrawText(0, 0, true)
				if textcount > 0 then
					--spEcho(Spring.GetDrawFrame(),"layer text rebuilt", layername)
					Layer.textDisplayList = gl.CreateList(function()
						font:Begin()
						Layer:DrawText(0, 0)
...
local function makeSliderList(sliderListConfig)
	...
	if sliderListConfig.parent == nil then
		container = metaElement:NewUiElement({
			name = sliderListConfig.name,
			left = left - 4,
			bottom = bottom - 4,
			right = left + width + 4,
			top = bottom + (numelements + 1) * sliderheight + 4,
			color1 = { 1, 1, 1, 0.5 },
			color2 = { 0, 0, 0, 1.0 },
			opacity = 0.2,
			parent = ROOT,
			...
			if unitDef.buildOptions[idx] then
				...
				local newbtn = metaElement:NewUiUnit({
					...
					MouseEvents = {
						left = function(obj)
							local instanceKeys = ""
							for i, instanceKey in ipairs(obj.instanceKeys) do
								instanceKeys = instanceKeys .. "," .. tostring(instanceKey)
							end
...
local function makeRectRoundVBO(name)
	...
	}, 1024, "rectRoundVBO" .. (name or ""))
	if rectRoundVBO == nil then
		goodbye("Failed to create rectRoundVBO")
	end
	...
	for i = 1, 0 do
		...
		local VBOData = {
			...
			0,
			1,
			1, --mathRandom() , mathRandom(), mathRandom() , mathRandom() ,
			mathRandom(),
			mathRandom(),
			mathRandom(),
			mathRandom(),
...
Draw.Element = function(VBO, instanceID, z, px, py, sx, sy, tl, tr, br, bl, ptl, ptr, pbr, pbl, opacity, color1, color2, bgpadding)
	local opacity = opacity or Engine.Unsynced.GetConfigFloat("ui_opacity", 0.7)
	...
	local tileopacity = Engine.Unsynced.GetConfigFloat("ui_tileopacity", 0.014)
	local bgtexScale = Engine.Unsynced.GetConfigFloat("ui_tilescale", 7)
...
Draw.Unit = function(VBO, instanceID, z, px, py, sx, sy, cs, tl, tr, br, bl, zoom, borderSize, borderOpacity, texture, radarTexture, groupTexture, price, queueCount)
	...
	local unitpic = Draw.TexturedRectRound(VBO, nil, z + 0.001, px, py, sx, sy, cs, tl, tr, br, bl, zoom, nil, nil, texture)
	...
	local lighten = Draw.RectRoundCircle(
		VBO,
		nil,
		z + 0.004,
		px + halfSize,
	...
	if borderSize > 0 then
		elementIDs[#elementIDs + 1] = Draw.RectRoundCircle(VBO, nil, z + 0.005, px + halfSize, py + halfSize, halfSize, cs * 0.7, halfSize - borderSize, { 1, 1, 1, borderOpacity }, { 1, 1, 1, borderOpacity })
	end
	--gl.Blending(GL.SRC_ALPHA, GL.ONE_MINUS_SRC_ALPHA)
	...
	if groupTexture then
		...
		elementIDs[#elementIDs + 1] = Draw.TexRect(VBO, nil, z + 0.006, px, sy - iconSize, px + iconSize, sy, groupTexture, { 1, 1, 1, 0.9 })
	end
	if radarTexture then
		...
		elementIDs[#elementIDs + 1] = Draw.TexRect(VBO, nil, z + 0.006, sx - iconPadding - iconSize, py + iconPadding, sx - iconPadding, py + iconPadding + iconSize, radarTexture, { 1, 1, 1, 0.9 })
	end
	local cnt = 0
...
function widget:Update()
	...
	if i % 100 == 0 then -- todo, if a layer doesnt have text then it can be batch drawn under a layer that does
		...
		for a = 1, #LayerDrawOrder do
			...
			if aclear then
				nonoverlapping[LayerDrawOrder[a]] = true
			end
		end
...
local function DrawLayer(layername)
	...
	if Layer.VBO.dirty then
		uploadAllElements(Layer.VBO)
	end
...
function widget:DrawScreen()
	...
	local mx, my, left, middle, right = Engine.Unsynced.GetMouseState()
	uiUpdate(mx, my, left, middle, right)
	RefreshText()

	if rectRoundVBO.dirty then
		uploadAllElements(rectRoundVBO)
	end -- do updates!
	...
	for layername, _ in pairs(nonoverlapping) do
		local Layer = Layers[layername]
		if Layer.textDisplayList then
			gl.CallList(Layer.textDisplayList)
		end
	end
