...
local _, _, mySpec, myTeamID = Engine.Shared.GetPlayerInfo(myPlayerID, false)
...
local function colourNames(teamID, blink)
	...
	if teamID ~= nil then
		...
		nameColourR, nameColourG, nameColourB = Engine.Unsynced.GetTeamColor(teamID)
	end
...
local function findPlayerName(playerID)
		...
		for _, player in ipairs(myTeamPlayersOrder) do
			if player.id == playerID then
				if player.name ~= nil then
					...
					tname = select(1, Engine.Shared.GetPlayerInfo(playerID, false))
	...
	tname = ((WG.playernames and WG.playernames.getPlayername) and WG.playernames.getPlayername(playerID)) or Engine.Shared.GetPlayerInfo(playerID, false)
	if not tname then
...
local function getHumanCountWithinAllyTeam(allyTeamID)
	local myTeamList = Engine.Shared.GetTeamList(allyTeamID)
	local count = 0
	for _, teamID in ipairs(myTeamList) do
		local _, _, _, isAiTeam = Engine.Shared.GetTeamInfo(teamID, false)
	...
	TeamPlacementUI = glCreateList(function()
		...
		if not amIunlocked and (current_playerID ~= myPlayerID) then
			...
			if current_playerID > -1 then
				...
				local tTeamID = select(4, Engine.Shared.GetPlayerInfo(current_playerID, false))
		...
		if current_playerID > -1 and next_playerID > -1 then
			...
			local tTeamID = select(4, Engine.Shared.GetPlayerInfo(next_playerID, false))
			local text = colourNames(tTeamID, false) .. tname
		...
		for i, data in ipairs(myTeamPlayersOrder) do
			...
			local _, active, _, playerTeamID, _, ping, _, _, rank, _, customtable = Engine.Shared.GetPlayerInfo(playerID, true)
...
local function progressQueueLocally(shift) -- only for dev UI testing of DOM
	...
	if myTeamPlayersOrder ~= nil then
		...
		elseif next_playerID == myPlayerID then
			Engine.Unsynced.PlaySoundFile("beep6", 1, "ui")
		elseif myTurn then
			myTurn = false
		end
	end
...
function widget:GameSetup(state, ready, playerStates)
	...
	if not spec and not ihavejoined and Engine.Shared.GetGameRulesParam("player_" .. Engine.Unsynced.GetLocalPlayerID() .. "_joined") == nil then
...
function widget:Initialize()
	...
	local xn, zn, xp, zp = Engine.Shared.GetAllyTeamStartBox(myAllyTeamID)
...
function widget:DrawScreen()
	...
	if Engine.Shared.GetGameRulesParam("all_players_joined") == 1 and not gameStarting and auto_ready and not auto_ready_disable then
		...
		if not mySpec then
			...
			if draftMode ~= "fair" then
				if myTurn and currentTurnTimeout and os.clock() >= currentTurnTimeout and not startPointChosen then
					Engine.Unsynced.SendLuaRulesMsg("skip_my_turn")
					myTurn = false
				end

				if voteSkipTurnTimeout and os.clock() >= voteSkipTurnTimeout then
					Engine.Unsynced.SendLuaRulesMsg("vote_skip_turn")
	...
	if not mySpec and draftMode ~= "disabled" then
		...
		if fairTimeout and os.clock() >= fairTimeout and not ihavejoined_fair then
			Engine.Unsynced.SendLuaRulesMsg("i_have_joined_fair")
		...
		if voteConTimeout and os.clock() >= voteConTimeout and ihavejoined_fair then
			...
			if not myAllyTeamJoined then
				Engine.Unsynced.SendLuaRulesMsg("vote_wait_too_long")
			end
			voteConTimeout = nil
		end
		if reloadedDraftMode and os.clock() >= reloadedDraftMode then
			reloadedDraftMode = nil
			Engine.Unsynced.SendLuaRulesMsg("send_me_the_info_again")
			draftModeInited()
		end
	end
	-- DOM end
