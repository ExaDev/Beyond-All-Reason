...
function widget:Initialize()
	if not Engine.Unsynced.GetSpectatingState() and not isSinglePlayer then
...
local function GetAllyTeamStats(allyTeamID)
	...
	if not allyTeamName[allyTeamID] then
		local _, playerID, _, isAiTeam = Engine.Shared.GetTeamInfo(teamlist[1], false)
		local name = (WG.playernames and WG.playernames.getPlayername) and WG.playernames.getPlayername(playerID) or Engine.Shared.GetPlayerInfo(playerID, false)
		allyTeamName[allyTeamID] = ColorString(Engine.Unsynced.GetTeamColor(teamlist[1])) .. name
	end
