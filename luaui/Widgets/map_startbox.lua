	...
	elseif string.sub(msg, 1, 20) == "aiPlacementComplete:" then
		...
		if teamID and x and z then
			...
			if x == 0 and z == 0 then
				...
				aiPlacementStatus[teamID] = false
				invalidatePosCacheEntry(teamID)
				aiLocationI18NTable.playerName = spGetPlayerInfo(Engine.Unsynced.GetLocalPlayerID(), false)
				aiLocationI18NTable.aiName = getAIName(teamID)
				Engine.Unsynced.SendMessage(BAR.I18N("ui.startbox.aiStartLocationRemoved", aiLocationI18NTable))
			else
				...
				aiLocationI18NTable.playerName = spGetPlayerInfo(Engine.Unsynced.GetLocalPlayerID(), false)
				aiLocationI18NTable.aiName = getAIName(teamID)
				Engine.Unsynced.SendMessage(BAR.I18N("ui.startbox.aiStartLocationChanged", aiLocationI18NTable))
			end
