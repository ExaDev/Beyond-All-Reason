...
local function DrawSmoothness()
	...
	for areaHash, areaInfo in pairs(areaDecals) do
		...
		if Engine.Unsynced.IsSphereInView(areaInfo.x, areaInfo.y, areaInfo.z, 128) then
			gl.PushMatrix()
...
local function UnitScriptDecal(unitID, unitDefID, whichDecal, posx, posz, heading)
	...
	if Engine.Shared.ValidUnitID(unitID) and Engine.Shared.GetUnitIsDead(unitID) == false and UnitScriptDecals[unitDefID] and UnitScriptDecals[unitDefID][whichDecal] then
		...
		if false then -- old reliable method
			...
			decalCache[14] = Engine.Shared.GetGroundHeight(posx, posz)
		...
		if curFrame > 0 then
			...
			for _, entry in ipairs(pendingRestore.decals) do
				...
				if step == 13 then
					...
					local posy = Engine.Shared.GetGroundHeight(posx, posz) or 0
					vboEntry = {
						entry[1],
						entry[2],
						entry[3],
