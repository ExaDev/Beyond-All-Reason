...
local function log(level, str, ...)
	...
	Engine.Shared.Log(widget:GetInfo().name, LOG.NOTICE, str)
end
...
local function getGameTime()
	if gameTimer ~= nil then
		return Engine.Unsynced.DiffTimers(Engine.Unsynced.GetTimer(), gameTimer, true)
	end
end

local function getRunTestsTime()
	if runTestsTimer ~= nil then
		return Engine.Unsynced.DiffTimers(Engine.Unsynced.GetTimer(), runTestsTimer, true)
	end
end

local function getTestTime()
	if testTimer ~= nil then
		return Engine.Unsynced.DiffTimers(Engine.Unsynced.GetTimer(), testTimer, true)
	end
end
...
local function startTests(patterns)
	...
	if not Engine.Shared.GetGameRulesParam("isSyncedProxyEnabled") then
	...
	if not Engine.Shared.IsDevLuaEnabled() then
	...
	if Engine.Shared.GetModOptions().deathmode ~= "neverend" and not Engine.Shared.GetGameRulesParam("testEndConditionsOverride") then
	...
	if spGetGameFrame() < 1 and not Engine.Shared.GetGameRulesParam("testEnvironmentStarting") then
	...
	if #neededActions > 0 then
		if not queuedStartTests then
			...
			for _, action in ipairs(neededActions) do
				...
				Engine.Unsynced.SendCommands(action[1])
			end
	...
	runTestsTimer = Engine.Unsynced.GetTimer()
end
...
local function createNestedProxy(prefix, path)
	return setmetatable({}, {
		...
		__call = function(_, ...)
			...
			Engine.Unsynced.SendLuaRulesMsg(prefix .. serializedFn)
...
SyncedRun = function(fn, timeout)
	...
	Engine.Unsynced.SendLuaRulesMsg(Proxy.PREFIX.RUN .. serializedFn)
...
Test = {
	...
	waitTime = function(milliseconds, timeout)
		...
		Test.waitUntil(function()
			return Engine.Unsynced.DiffTimers(Engine.Unsynced.GetTimer(), startTimer, true) >= milliseconds
	...
	clearMap = function()
		SyncedRun(function()
			for _, unitID in ipairs(Engine.Shared.GetAllUnits()) do
				Engine.Synced.DestroyUnit(unitID, false, true, nil, false)
			end
...
local function step()
	...
	if activeTestState.coroutine == nil then
		...
		if success then
			...
			testTimer = Engine.Unsynced.GetTimer()
		else
			finishTest({
...
function widget:Initialize()
	...
	gameTimer = Engine.Unsynced.GetTimer()
end
