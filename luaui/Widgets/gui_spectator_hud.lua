...
local gaiaID = Engine.Shared.GetGaiaTeamID()
local gaiaAllyID = select(6, Engine.Shared.GetTeamInfo(gaiaID, false))
...
local function checkAndUpdateHaveFullView()
	...
	haveFullView = select(2, Engine.Unsynced.GetSpectatingState())
...
local function buildUnitCache()
	...
	unitCache.reclaimerUnits = {
		...
		update = function(unitID, value)
			...
			local metalMake, metalUse, energyMake, energyUse = Engine.Shared.GetUnitResources(unitID)
			if metalMake then
				if value[1] then
	...
	for _, allyID in ipairs(Engine.Shared.GetAllyTeamList()) do
		if allyID ~= gaiaAllyID then
			...
			for _, teamID in ipairs(teamList) do
				...
				local unitIDs = Engine.Shared.GetTeamUnits(teamID)
				for i = 1, #unitIDs do
	...
	for _, allyID in ipairs(Engine.Shared.GetAllyTeamList()) do
		if allyID ~= gaiaAllyID then
			...
			for _, teamID in ipairs(teamList) do
				...
				local playerID = Engine.Shared.GetPlayerList(teamID, false)
				if playerID and playerID[1] then
					-- it's a player
					playerName = select(1, Engine.Shared.GetPlayerInfo(playerID[1], false))
					playerName = ((WG.playernames and WG.playernames.getPlayername) and WG.playernames.getPlayername(playerID[1])) or playerName
				else
					local aiName = Engine.Shared.GetGameRulesParam("ainame_" .. teamID)
					if aiName then
						-- it's AI
						playerName = aiName
					else
				...
				local teamColor = { Engine.Unsynced.GetTeamColor(teamID) }
	...
	for _, allyID in ipairs(Engine.Shared.GetAllyTeamList()) do
		if allyID ~= gaiaAllyID then
			...
			if teamList and teamList[1] then
				local colorCaptain = (playerData[teamList[1]] and playerData[teamList[1]].color) or { Engine.Unsynced.GetTeamColor(teamList[1]) }
...
local function buildMetricsEnabled()
	...
	for _, metric in ipairs(metricsAvailable) do
		...
		if addMetric then
			...
			metricEnabled.title = BAR.I18N(i18nTitleKey)
			local i18nTooltipKey = "ui.spectator_hud." .. metricEnabled.key .. "_tooltip"
			metricEnabled.tooltip = BAR.I18N(i18nTooltipKey)
			index = index + 1
		end
	end
end
...
local function getOneStat(statKey, teamID)
	...
	if statKey == metricKeys.metalIncome then
		result = select(4, Engine.Shared.GetTeamResources(teamID, "metal")) or 0
	...
	elseif statKey == metricKeys.energyIncome then
		result = select(4, Engine.Shared.GetTeamResources(teamID, "energy")) or 0
	...
	elseif statKey == metricKeys.metalProduced then
		...
		local _, metalProduced, _, _, _ = Engine.Shared.GetTeamResourceStats(teamID, "m")
		result = metalProduced
	elseif statKey == metricKeys.energyProduced then
		local _, energyProduced, _, _, _ = Engine.Shared.GetTeamResourceStats(teamID, "e")
		result = energyProduced
	elseif statKey == metricKeys.metalExcess then
		local _, _, metalExcess, _, _ = Engine.Shared.GetTeamResourceStats(teamID, "m")
		result = metalExcess
	elseif statKey == metricKeys.energyExcess then
		local _, _, energyExcess, _, _ = Engine.Shared.GetTeamResourceStats(teamID, "e")
		result = energyExcess
	...
	elseif statKey == metricKeys.damageDealt then
		local historyMax = Engine.Shared.GetTeamStatsHistory(teamID)
		local statsHistory = Engine.Shared.GetTeamStatsHistory(teamID, historyMax)
		local damageDealt = 0
...
local function init()
	...
	viewScreenWidth, viewScreenHeight = Engine.Unsynced.GetViewGeometry()
...
function widget:GameFrame(frameNum)
	...
	if (frameNum > 0) and not teamOrder then
		...
		local _, rotY, _ = Engine.Unsynced.GetCameraRotation()
