...
local hideSpecChat = tonumber(Engine.Unsynced.GetConfigInt("HideSpecChat", 0) or 0) == 1
...
function widget:Update(dt)
	...
	if uiSec > 0.5 then
		uiSec = 0
		if hideSpecChat ~= tonumber(Engine.Unsynced.GetConfigInt("HideSpecChat", 0) or 0) == 1 then
			hideSpecChat = tonumber(Engine.Unsynced.GetConfigInt("HideSpecChat", 0) or 0) == 1
		end
...
function widget:SetConfigData(data)
	if Engine.Shared.GetGameFrame() > 0 and data.messageLines ~= nil then
