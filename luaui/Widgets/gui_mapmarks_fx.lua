...
local ownPlayerID = Engine.Unsynced.GetLocalPlayerID()
...
function widget:DrawWorldPreUnit()
	...
	while i <= commandCount do
		...
		if not cmd then
			...
			if currentTime - cmd.osClock > totalDuration or (cmd.cmdType == "map_draw" and mapDrawNicknameTime[cmd.playerID] ~= nil and cmd.osClock < mapDrawNicknameTime[cmd.playerID]) then
				commands[i] = commands[commandCount]
				commands[commandCount] = nil
				commandCount = commandCount - 1
			else
	...
	for j = 1, commandCount do
		...
		if cmd.spec and cmd.playerID and cmd.playerID ~= ownPlayerID then
			...
			if cmdType == "map_draw" or (cmdType == "map_erase" and cmd.osClock >= (mapEraseNicknameTime[cmd.playerID] or 0)) then
				local iconAlpha = a * glowAlpha * nicknameOpacityMultiplier
				local is = 11
				if cmdType == "map_draw" then
					local n = pencilN
					pencilBatch[n + 1] = cmd.x
					pencilBatch[n + 2] = cmd.y
					pencilBatch[n + 3] = cmd.z
					pencilBatch[n + 4] = is
					pencilBatch[n + 5] = cr
					pencilBatch[n + 6] = cg
					pencilBatch[n + 7] = cb
					...
					local n = eraserN
					eraserBatch[n + 1] = cmd.x
					eraserBatch[n + 2] = cmd.y
					eraserBatch[n + 3] = cmd.z
					eraserBatch[n + 4] = is
					eraserBatch[n + 5] = cr
					eraserBatch[n + 6] = cg
					eraserBatch[n + 7] = cb
				...
				nickN = nickN + 1
				local nn = (nickN - 1) * 4
				nickBatch[nn + 1] = cmd.x
				nickBatch[nn + 2] = cmd.y
				nickBatch[nn + 3] = cmd.z
