...
local spGetGrass = Engine.Shared.GetGrass
...
local function scanUnitPositions(gf)
	...
	local gh = Engine.Shared.GetGroundHeight(cx, cz) or 0
...
local function makeGrassPatchVBO(grassPatchSize) -- grassPatchSize = 1|4, see the commented out python section at the bottom to make a custom vbo
	...
	local VBOLayout = { { id = 0, name = "position", size = 3 }, { id = 1, name = "normal", size = 3 }, { id = 2, name = "stangent", size = 3 }, { id = 3, name = "ttangent", size = 3 }, { id = 4, name = "texcoords0", size = 2 }, { id = 5, name = "texcoords1", size = 2 }, { id = 6, name = "pieceindex", size = 1 } } --khm, this should be unsigned int
...
local function testForGrass(mx, mz)
	if grassConfig.obeyGrassMap then
		...
		local gx, gy, gz, gs = Engine.Shared.GetGroundNormal(mx, mz)
		local gh = spGetGroundHeight(mx, mz)
		if (gh > grassConfig.grassMinHeight) and (gh < grassConfig.grassMaxHeight) and (gy > grassConfig.grassMaxSlope) then
			return gh
		else
			return nil
		end
...
local function updateGrassInstanceVBO(wx, wz, size, sizemod, vboOffset)
	...
	if placementMode then
		_, _, _, shift = Engine.Unsynced.GetModKeyState()
	end
	if sizemod < 1 then
...
function widget:VisibleUnitAdded(unitID, unitDefID, unitTeam)
	if processChanges and not placementMode and buildingRadius[unitDefID] and not unitGrassRemovedHistory[unitID] then
		local isBuilding = Engine.Shared.GetUnitIsBeingBuilt(unitID)
		if isBuilding then
...
function widget:GameFrame(gf)
	...
	if gf == 85 then
		...
		for _, unitID in pairs(allUnits) do
			if isCommander[Engine.Shared.GetUnitDefID(unitID)] then
...
local function savegrassCmd(_, _, params)
	...
	texture = BAR.Utilities.NewTGA(mathFloor(mapSizeX / grassConfig.patchResolution), mathFloor(mapSizeZ / grassConfig.patchResolution), 1)
	local offset = 0
	...
	local success = BAR.Utilities.SaveTGA(texture, filename)
	if success then
		spEcho("Saving grass map image failed", filename, success)
	end
end
...
local function getWindSpeed()
	windDirX, _, windDirZ, _ = Engine.Shared.GetWind()
...
function widget:DrawWorldPreUnit()
	...
	local mapDrawMode = Engine.Unsynced.GetMapDrawMode()
	...
	if camHeight < fadeEnd and grassVAO ~= nil and grassDataLen > 0 then
		...
		if placementMode and Engine.Shared.GetGameFrame() % 30 == 0 then
