...
local function parseCEGExpression(inputCegString, numFloats, countIndex, damage)
	...
	for expression in string.gmatch(inputCegString, "([^,]+)") do
		...
		while i <= #expression do
			...
			if char == " " then
				i = i + 1
			elseif char == "i" or char == "r" or char == "d" or char == "m" or char == "k" or char == "s" or char == "y" or char == "x" or char == "a" or char == "p" or char == "q" or char == "+" then
				currentOp = char
				i = i + 1
...
local function isExplosionGenerator(value)
	if type(value) == "string" then
		if true then
			return true
		end -- FIXME !!!!! names are only valid after parsing all of them
...
local function isValidTexture(value)
	if (type(value) == "string" and (projectileTexures[value] or projectileTexures[string.lower(value)])) or value == "none" or value == "nil" or value == "null" then
		return true
	else
					...
					if cegDefTemplate[k] then
						if cegDefTemplate[k].validator then
							...
							if not res then
								...
								local msg = string.format("Error: CEG {%s = {%s = {%s = %s ,...}}} : %s", tostring(cegName), tostring(spawnername), tostring(k), ((type(v) == "string") and '"' or "") .. tostring(v) .. ((type(v) == "string") and '"' or ""), tostring(err))
							...
							if cegDefTemplate[k2] then
								if cegDefTemplate[k2].validator then
									...
									if not res then
										...
										local msg = string.format('Error: CEG {%s = {%s = {properties = {%s = "%s"  ...}}}} is invalid: %s', tostring(cegName), tostring(spawnername), tostring(k2), ((type(v2) == "string") and '"' or "") .. tostring(v2) .. ((type(2) == "string") and '"' or ""), tostring(err))
