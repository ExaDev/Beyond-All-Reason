...
function widget:Update()
	...
	local isSpec = select(3, Engine.Shared.GetPlayerInfo(Engine.Unsynced.GetLocalPlayerID(), false))
	if isSpec or t > 10 then
		widgetHandler:RemoveWidget()
		return
	end
	if t > 0 then
		local unitArray = Engine.Shared.GetTeamUnits(Engine.Unsynced.GetLocalTeamID())
