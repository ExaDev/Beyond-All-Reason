...
function widget:KeyPress(key, mods, isRepeat)
	if not mods.alt and mods.shift then
		if key ~= KEYSYMS.UP and key ~= KEYSYMS.DOWN and key ~= KEYSYMS.LEFT and key ~= KEYSYMS.RIGHT then -- Don't ignore arrow keys
			local x, y, leftPressed, middlePressed, rightPressed, offscreen = Engine.Unsynced.GetMouseState()
			if leftPressed then
				local idx, cmd_id, cmd_type, cmd_name = Engine.Unsynced.GetActiveCommand()
