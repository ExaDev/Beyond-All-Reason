...
local spLoadCmdColorsConfig = Engine.Unsynced.LoadCmdColorsConfig
...
local function InitGL4()
	...
	if not shader then
		Engine.Shared.Echo("[CommandsFX2] GL4 shader failed: " .. tostring(gl.GetShaderLog()))
		return false
	end
...
local function loadTeamColors()
	...
	for i = 1, #teams do
		local r, g, b = Engine.Unsynced.GetTeamColor(teams[i])
		local min = 0.12
...
local function resetEnabledTeams()
	...
	for _, teamID in ipairs(t) do
		if not filterAIteams or not select(4, Engine.Shared.GetTeamInfo(teamID, false)) then
...
function widget:Initialize()
	...
	if not InitGL4() then
		Engine.Shared.Echo("[CommandsFX2] GL4 initialization failed, disabling widget")
...
function widget:Update(dt)
	...
	if sec > lastUpdate + 0.1 then
		...
		for fp in pairs(queueShareCache) do
			queueShareCache[fp] = nil
		end
