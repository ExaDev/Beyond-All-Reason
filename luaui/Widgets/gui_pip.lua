...
local isSpectator = Engine.Unsynced.GetSpectatingState()
...
local function getActionHotkey(action)
	local currentKeyboardLayout = Engine.Unsynced.GetConfigString("KeyboardLayout", "qwerty")
	local hotkeys = Engine.Unsynced.GetActionHotKeys(action)
...
config = {
	...
	drawCommandFX = Engine.Unsynced.GetConfigInt("PipDrawCommandFX", 1) == 1, -- Show brief fading command lines when orders are given (like Commands FX widget)
	...
	leftButtonPansCamera = isMinimapMode and (Engine.Unsynced.GetConfigInt("MinimapLeftClickMove", 1) == 1) or false,
	maximizeSizemult = 1.25,
	screenMargin = 0.00,
	drawProjectiles = true,
	zoomToCursor = true,
	altKeyRequiredForZoom = Engine.Unsynced.GetConfigInt("PipAltKeyRequiredForZoom", 1) == 1, -- When true, scrolling over the PIP only zooms if ALT is held (otherwise passes through to the game)
	mapEdgeMargin = 0,
	...
	minimapModeMaxHeight = Engine.Unsynced.GetConfigFloat("MinimapMaxHeight", 0.32), -- Shared with gui_minimap.lua via ConfigFloat
...
local render = {
	uiScale = tonumber(Engine.Unsynced.GetConfigFloat("ui_scale", 1) or 1),
	vsx = nil,
	vsy = nil,
	widgetScale = nil,
	usedButtonSize = nil,
...
render.vsx, render.vsy = Engine.Unsynced.GetViewGeometry()
...
function pipTV.BuildHotspots(now)
	...
	for i = 1, pipTV.eventCount do
		...
		if ev then
			...
			if age < decayTime and (not losFilter or Engine.Shared.IsPosInLos(ev.x, 0, ev.z, losFilter)) then
...
function pipTV.ScanAnticipation(now)
	...
	for i = 1, #allUnits do
		...
		local defID = Engine.Shared.GetUnitDefID(uID)
		if defID then
			local teamID = Engine.Shared.GetUnitTeam(uID)
			local allyTeam = teamID and Engine.Shared.GetTeamAllyTeamID(teamID)
			local x, _, z = Engine.Shared.GetUnitBasePosition(uID)
			if x and allyTeam then
				...
				if cache.isCommander[defID] then
					local hp, maxHP = Engine.Shared.GetUnitHealth(uID)
...
function pipTV.DirectorTick(dt)
	...
	if not dir.effectiveGameOver and now - dir.lastAliveCheck >= 2.0 then
		...
		if gf > 30 * 30 then -- Only check after first 30 seconds (avoid false positive at spawn)
			local gaiaAT = select(6, Engine.Shared.GetTeamInfo(Engine.Shared.GetGaiaTeamID()))
			local aliveCount = 0
			...
			for a = 1, #allyTeams do
				...
				if atID ~= gaiaAT then
					...
					local teams = Engine.Shared.GetTeamList(atID)
					for t = 1, #teams do
						local _, _, isDead = Engine.Shared.GetTeamInfo(teams[t], false)
						if not isDead then
...
cache.gaiaAllyTeamID = select(6, Engine.Shared.GetTeamInfo(gaiaTeamID))
...
do
	...
	for i = 1, #teamList do
		...
		local _, _, _, isAI = Engine.Shared.GetTeamInfo(teamID, false)
		if isAI then
			aiTeams[teamID] = true
...
local spFunc = {
	GetGroundHeight = Engine.Shared.GetGroundHeight,
	GetUnitsInRectangle = Engine.Shared.GetUnitsInRectangle,
	GetUnitPosition = Engine.Shared.GetUnitPosition,
	GetUnitBasePosition = Engine.Shared.GetUnitBasePosition,
	GetUnitTeam = Engine.Shared.GetUnitTeam,
	GetUnitDefID = Engine.Shared.GetUnitDefID,
	GetTeamInfo = Engine.Shared.GetTeamInfo,
	IsPosInLos = Engine.Shared.IsPosInLos,
	IsPosInRadar = Engine.Shared.IsPosInRadar,
	GetUnitLosState = Engine.Shared.GetUnitLosState,
	GetFeatureDefID = Engine.Shared.GetFeatureDefID,
	GetFeatureDirection = Engine.Shared.GetFeatureDirection,
	GetFeaturePosition = Engine.Shared.GetFeaturePosition,
	GetFeatureTeam = Engine.Shared.GetFeatureTeam,
	GetFeaturesInRectangle = Engine.Shared.GetFeaturesInRectangle,
	IsUnitSelected = Engine.Unsynced.IsUnitSelected,
	GetUnitHealth = Engine.Shared.GetUnitHealth,
	GetMouseState = Engine.Unsynced.GetMouseState,
	GetProjectilesInRectangle = Engine.Shared.GetProjectilesInRectangle,
	GetProjectilePosition = Engine.Shared.GetProjectilePosition,
	GetProjectileDefID = Engine.Shared.GetProjectileDefID,
	GetProjectileTarget = Engine.Shared.GetProjectileTarget,
	GetProjectileOwnerID = Engine.Shared.GetProjectileOwnerID,
	GetProjectileVelocity = Engine.Shared.GetProjectileVelocity,
	GetUnitCommands = Engine.Shared.GetUnitCommands,
	GetPlayerInfo = Engine.Shared.GetPlayerInfo,
	GetUnitIsStunned = Engine.Shared.GetUnitIsStunned,
	GetTeamAllyTeamID = Engine.Shared.GetTeamAllyTeamID,
	GetUnitSelfDTime = Engine.Shared.GetUnitSelfDTime,
	GetSelectedUnitsCount = Engine.Unsynced.GetSelectedUnitsCount,
}
...
do
	...
	mapInfo.minGroundHeight, mapInfo.maxGroundHeight = Engine.Shared.GetGroundExtremes()
...
if mapInfo.isLava or mapInfo.hasWater then
	local initLavaLevel = Engine.Shared.GetGameRulesParam("lavaLevel")
...
local buttons = {
	{
		...
		command = "pip_copy",
		OnPress = function()
			local sizex, sizez = Engine.Unsynced.GetWindowGeometry()
			local _, pos = Engine.Unsynced.TraceScreenRay(sizex / 2, sizez / 2, true)
		...
		OnPress = function()
			...
			if pos and pos[2] > -10000 then
				...
				if config.switchInheritsTracking and interactionState.areTracking and #interactionState.areTracking > 0 then
					...
					Engine.Unsynced.SendCommands("track")
					-- Then track the PIP units in main view
					for i = 1, #interactionState.areTracking do
						Engine.Unsynced.SendCommands("track " .. interactionState.areTracking[i])
					end
				else
				...
				Engine.Unsynced.SetCameraTarget(pipCameraTargetX, 0, pipCameraTargetZ, config.switchTransitionTime)
	...
	},
	{
		...
		OnPress = function()
			...
			if state.losViewEnabled then
				...
				state.losViewAllyTeam = Engine.Unsynced.GetLocalAllyTeamID()
				...
				if cameraState.mySpecState then
					...
					for _, uID in ipairs(allUnits) do
						...
						if defID and cache.isBuilding[defID] then
							local uTeam = Engine.Shared.GetUnitTeam(uID)
							local uAllyTeam = Engine.Shared.GetTeamAllyTeamID(uTeam)
							if uAllyTeam ~= scanAllyTeam then
								-- Only ghost buildings the viewed allyteam has ever seen
								local losBits = Engine.Shared.GetUnitLosState(uID, scanAllyTeam, true)
	...
	},
	{
		...
		OnPress = function()
			if interactionState.trackingPlayerID then
				...
				local _, _, isSpec = spFunc.GetPlayerInfo(Engine.Unsynced.GetLocalPlayerID(), false)
				...
				if isSpec then
					...
					if #teammates > 0 then
						...
						if targetPlayerID ~= Engine.Unsynced.GetLocalPlayerID() then
							interactionState.trackingPlayerID = targetPlayerID
...
for i = 1, #teamList do
	...
	teamColors[tID] = { Engine.Unsynced.GetTeamColor(tID) }
	teamAllyTeamCache[tID] = select(6, Engine.Shared.GetTeamInfo(tID, false))
end
...
local function InitGL4Icons()
	...
	if not gl.GetVAO or not gl.GetVBO then
		Engine.Shared.Echo("[PIP] GL4 icons: VAO/VBO not available, falling back to legacy")
		return
	end

	if not Engine.Unsynced.GetIconData then
		Engine.Shared.Echo("[PIP] GL4 icons: Spring.GetIconData not available, falling back to legacy")
		return
	end

	local defaultIconData = Engine.Unsynced.GetIconData("default")
	if not defaultIconData or not defaultIconData.atlasTexCoords then
		Engine.Shared.Echo("[PIP] GL4 icons: Engine icon atlas data not available, falling back to legacy")
		return
	end

	local iconsTexInfo = gl.TextureInfo("$icons")
	if not iconsTexInfo or iconsTexInfo.xsize <= 0 then
		Engine.Shared.Echo("[PIP] GL4 icons: $icons texture not available, falling back to legacy")
		return
	end
	...
	for uDefID, uDef in pairs(UnitDefs) do
		...
		local iconInfo = Engine.Unsynced.GetIconData(iconName)
	...
	if not vbo then
		Engine.Shared.Echo("[PIP] GL4 icons: Failed to create VBO")
		gl4Icons.atlas = nil
		return
	end
	...
	local vao = gl.GetVAO()
	if not vao then
		Engine.Shared.Echo("[PIP] GL4 icons: Failed to create VAO")
		vbo:Delete()
		gl4Icons.atlas = nil
		gl4Icons.vbo = nil
		return
	...
	if not bldgVbo then
		Engine.Shared.Echo("[PIP] GL4 icons: Failed to create building VBO")
		vao:Delete()
		vbo:Delete()
		gl4Icons.atlas = nil
		gl4Icons.vbo = nil
	...
	if not bldgVao then
		Engine.Shared.Echo("[PIP] GL4 icons: Failed to create building VAO")
		bldgVbo:Delete()
		vao:Delete()
		vbo:Delete()
		gl4Icons.atlas = nil
	...
	if not slowVbo then
		Engine.Shared.Echo("[PIP] GL4 icons: Failed to create slow mobile VBO")
		bldgVao:Delete()
		bldgVbo:Delete()
		vao:Delete()
		vbo:Delete()
	...
	if not slowVao then
		Engine.Shared.Echo("[PIP] GL4 icons: Failed to create slow mobile VAO")
		slowVbo:Delete()
		bldgVao:Delete()
		bldgVbo:Delete()
		vao:Delete()
	...
	if not shader then
		Engine.Shared.Echo("[PIP] GL4 icons: Shader compilation failed: " .. tostring(gl.GetShaderLog()))
		vao:Delete()
		vbo:Delete()
		gl4Icons.atlas = nil
		gl4Icons.vbo = nil
...
local function InitGL4Primitives()
	...
	if not cShader then
		Engine.Shared.Echo("[PIP] GL4 circle shader failed: " .. tostring(gl.GetShaderLog()))
		-- cleanup all
		nlVao:Delete()
		nlVbo:Delete()
		clVao:Delete()
	...
	if not qShader then
		Engine.Shared.Echo("[PIP] GL4 quad shader failed: " .. tostring(gl.GetShaderLog()))
	...
	if not lShader then
		Engine.Shared.Echo("[PIP] GL4 line shader failed: " .. tostring(gl.GetShaderLog()))
...
local function PipToWorldCoords(mx, my)
	...
	local minimapRotation = Engine.Unsynced.GetMiniMapRotation and Engine.Unsynced.GetMiniMapRotation() or 0
	...
	elseif pDefID and cache.weaponIsPlasma[pDefID] then
		...
		if trailColor then
			...
			if trailCount >= 2 then
				...
				if baseWidth < 1.5 * resScale then
					baseWidth = 1.5 * resScale
				end
...
local function ExpireExplosions()
	...
	local currentFrame = Engine.Shared.GetGameFrame()
	local i = 1
	while i <= n do
...
local function GiveNotifyingOrder(cmdID, cmdParams, cmdOpts)
	...
	Engine.Unsynced.GiveOrder(cmdID, cmdParams, cmdOpts.coded)
end
...
local function FindMyCommander()
	...
	local teamUnits = Engine.Shared.GetTeamUnits(myTeamID)
...
local function CalculateBuildDragPositions(startWX, startWZ, endWX, endWZ, buildDefID, alt, ctrl, shift)
	...
	local sx, sy, sz = Engine.Shared.Pos2BuildPos(buildDefID, startWX, spFunc.GetGroundHeight(startWX, startWZ), startWZ)

	-- For end position, snap it too to know the intended area
	local ex, ey, ez = Engine.Shared.Pos2BuildPos(buildDefID, endWX, spFunc.GetGroundHeight(endWX, endWZ), endWZ)
	...
	if shift and not alt then
		...
		for searchDist = searchStep, distance, searchStep do
			...
			local snappedX, _, snappedZ = Engine.Shared.Pos2BuildPos(buildDefID, testX, spFunc.GetGroundHeight(testX, testZ), testZ)
	...
	elseif shift and alt and not ctrl then
		...
		for row = 0, numZ - 1 do
			for col = 0, numX - 1 do
				...
				local snappedX, _, snappedZ = Engine.Shared.Pos2BuildPos(buildDefID, wx, spFunc.GetGroundHeight(wx, wz), wz)
...
local function IssueCommandAtPoint(cmdID, wx, wz, usingRMB, forceQueue, radius)
	...
	if Engine.Unsynced.GetInvertQueueKey() then
		shift = not shift
	end
	...
	if cmdID > 0 and cmdID ~= CMD.PATROL and cmdID ~= CMD.FIGHT then
		...
		if id and cmdID == CMD.ATTACK then
			if Engine.Unsynced.IsUnitAllied(id) then
		...
		if id and cmdID == CMD.RECLAIM and radius and radius > 0 then
			if not Engine.Unsynced.IsUnitAllied(id) then
	...
	if id then
				...
				if #transports > 1 then
					...
					if ux then
						...
						for i = 1, #transports do
							Engine.Shared.GiveOrderToUnit(transports[i], cmdID, { ux, uy, uz, smallRadius }, cmdOpts.coded)
						end
					end
				else
					-- Single transport, give direct unit target
					Engine.Shared.GiveOrderToUnit(transports[1], cmdID, { id }, cmdOpts.coded)
				end
			end
		else
		...
		if cmdID > 0 then
			...
			if radius and radius > 0 then
				...
				if cmdID == CMD.LOAD_UNITS then
					...
					for i = 1, #selectedUnits do
						local unitDefID = spFunc.GetUnitDefID(selectedUnits[i])
						if unitDefID and cache.isTransport[unitDefID] and (cache.transportCapacity[unitDefID] or 0) > 0 then
							Engine.Shared.GiveOrderToUnit(selectedUnits[i], cmdID, { wx, spFunc.GetGroundHeight(wx, wz), wz, radius }, cmdOpts.coded)
						end
					end
				else
			...
			GiveNotifyingOrder(cmdID, { wx, spFunc.GetGroundHeight(wx, wz), wz, Engine.Unsynced.GetBuildFacing() }, cmdOpts)
		end
	end
end
	...
	WG.minimap.setEngineMinimapFallback = function(value)
		...
		if not value and miscState.engineMinimapActive then
			...
			if miscState.baseMinimapIconScale then
				Engine.Unsynced.SendCommands("minimap unitsize " .. miscState.baseMinimapIconScale)
				Engine.Unsynced.SetConfigFloat("MinimapIconScale", miscState.baseMinimapIconScale)
...
function widget:Initialize()
	...
	if not shaders.los then
		Engine.Shared.Echo("PIP: Failed to compile LOS shader, LOS overlay will be disabled")
		Engine.Shared.Echo("PIP: Shader log: " .. (gl.GetShaderLog() or "no log"))
	...
	if not shaders.decal then
		Engine.Shared.Echo("PIP: Failed to compile decal shader")
		Engine.Shared.Echo("PIP: Shader log: " .. (gl.GetShaderLog() or "no log"))
	else
		InitGL4Decals()
	end
	...
	if shaders.decalBlit then
		...
		Engine.Shared.Echo("PIP: Failed to compile decal blit shader")
		Engine.Shared.Echo("PIP: Shader log: " .. (gl.GetShaderLog() or "no log"))
	end
	...
	if not shaders.minimapShading then
		Engine.Shared.Echo("PIP: Failed to compile minimap shading shader")
		Engine.Shared.Echo("PIP: Shader log: " .. (gl.GetShaderLog() or "no log"))
	end
	...
	if mapInfo.hasWater then
		...
		if not shaders.water then
			Engine.Shared.Echo("PIP: Failed to compile water shader")
			Engine.Shared.Echo("PIP: Shader log: " .. (gl.GetShaderLog() or "no log"))
		end
	end
	...
	gameHasStarted = (Engine.Shared.GetGameFrame() > 0)
	miscState.startX, _, miscState.startZ = Engine.Shared.GetTeamStartPosition(Engine.Unsynced.GetLocalTeamID())
			...
			if siblingGhosts then
				for gID, ghost in pairs(siblingGhosts) do
					if not ghostBuildings[gID] then
						local curDefID = Engine.Shared.GetUnitDefID(gID)
						local accept = true
						if curDefID then
							if curDefID ~= ghost.defID then
								accept = false
							else
								local curTeam = Engine.Shared.GetUnitTeam(gID)
	...
	do
		...
		local initScanIsSpec, initScanFullview = Engine.Unsynced.GetSpectatingState()
		if not initScanIsSpec then
			initScanAllyTeam = Engine.Unsynced.GetLocalAllyTeamID()
				...
				if defID and cache.isBuilding[defID] then
					...
					if uAllyTeam ~= initScanAllyTeam then
						if initScanIsSpec then
							...
							local losBits = Engine.Shared.GetUnitLosState(uID, initScanAllyTeam, true)
							if losBits and (losBits % 2 >= 1 or losBits % 8 >= 4) then
								local x, _, z = Engine.Shared.GetUnitBasePosition(uID)
								if x then
									ghostBuildings[uID] = { defID = defID, x = x, z = z, teamID = uTeam }
	...
	do
		local currentGameID = Game.gameID and Game.gameID or Engine.Shared.GetGameRulesParam("GameID")
	...
	if isMinimapMode then
		...
		miscState.oldMinimapGeometry = Engine.Unsynced.GetMiniMapGeometry()
		miscState.oldMinimapMinimized = Engine.Unsynced.GetConfigInt("MinimapMinimize", 0)
		miscState.oldMinimapDrawPings = Engine.Unsynced.GetConfigInt("MiniMapDrawPings", 1)
		...
		Engine.Unsynced.SetConfigInt("MiniMapDrawPings", 0)
...
function widget:ViewResize()
	...
	if isMinimapMode then
		...
		Engine.Unsynced.SendCommands(string.format("minimap geometry %d %d %d %d", math.floor(render.dim.l), math.floor(render.vsy - render.dim.t), math.floor(render.dim.r - render.dim.l), math.floor(render.dim.t - render.dim.b)))
	else
		...
		if uiState.inMinMode then
			...
			if dimsValid then
				...
				Engine.Shared.Echo("PIP: Detected invalid dimensions, resetting to default position")
				render.dim.l = 0.7
	...
	if savedDimsValid then
		...
		local sw, sh = Engine.Unsynced.GetWindowGeometry()
	...
	if miscState.engineMinimapActive then
		...
		Engine.Unsynced.SendCommands(string.format("minimap geometry %d %d %d %d", math.floor(render.dim.l), math.floor(render.vsy - render.dim.t), math.floor(w), math.floor(h)))
	end
...
function widget:PlayerChanged(playerID)
	...
	-- Update spec state
	cameraState.mySpecState = Engine.Unsynced.GetSpectatingState()
...
function widget:Shutdown()
	...
	if isMinimapMode then
		...
		if miscState.oldMinimapGeometry then
			Engine.Unsynced.SendCommands("minimap geometry " .. miscState.oldMinimapGeometry)
		end
		-- Restore original MiniMapDrawPings
		if miscState.oldMinimapDrawPings then
			Engine.Unsynced.SetConfigInt("MiniMapDrawPings", miscState.oldMinimapDrawPings)
		end
...
function widget:GetConfigData()
	...
	return {
		...
		gameID = Game.gameID or Engine.Shared.GetGameRulesParam("GameID"),
...
function widget:SetConfigData(data)
	...
	if data.pl and data.pr and data.pb and data.pt then
		...
		if isValidNumber(data.pl, 0, 1) and isValidNumber(data.pr, 0, 1) and isValidNumber(data.pb, 0, 1) and isValidNumber(data.pt, 0, 1) and data.pl < data.pr and data.pb < data.pt then -- Ensure left < right and bottom < top
			...
			if windowWidth >= minSize and windowHeight >= minSize and tempL >= 0 and tempR <= render.vsx and tempB >= 0 and tempT <= render.vsy then
				...
				Engine.Shared.Echo("PIP: Invalid saved dimensions detected, resetting to default position")
			end
		else
			-- Invalid position data - don't restore
			Engine.Shared.Echo("PIP: Corrupted position data detected, resetting to default position")
		end
	end
	...
	if data.tvEnabled ~= nil then
		...
		if data.tvEnabled and config.tvModeSpectatorsOnly and not Engine.Unsynced.GetSpectatingState() then
	...
	if data.minimapModeMaxHeight and type(data.minimapModeMaxHeight) == "number" and data.minimapModeMaxHeight > 0 and data.minimapModeMaxHeight <= 1 then
		if Engine.Unsynced.GetConfigFloat("MinimapMaxHeight", -1) == -1 then
			Engine.Unsynced.SetConfigFloat("MinimapMaxHeight", data.minimapModeMaxHeight)
		end
		config.minimapModeMaxHeight = Engine.Unsynced.GetConfigFloat("MinimapMaxHeight", 0.32)
	end
	-- leftButtonPansCamera now read from Spring ConfigInt "MinimapLeftClickMove"
	if data.leftButtonPansCamera ~= nil and Engine.Unsynced.GetConfigInt("MinimapLeftClickMove", -1) == -1 then
		-- Migrate old config data to new ConfigInt (one-time)
		config.leftButtonPansCamera = data.leftButtonPansCamera
		Engine.Unsynced.SetConfigInt("MinimapLeftClickMove", data.leftButtonPansCamera and 1 or 0)
	end
	...
	if Engine.Shared.GetGameFrame() > 0 or isSameGame then
...
local function DrawCommandFXOverlay()
	...
	local myTeamID = not useCommandColors and Engine.Unsynced.GetLocalTeamID() or nil
...
local function DrawBuildPreview(mx, my, iconRadiusZoomDistMult)
		...
		if metalSpots and not metalMap then
			...
			if mexBuildings then
				if not frameSel then
					frameSel = Engine.Unsynced.GetSelectedUnits()
				end
	...
	elseif activeCmdID and activeCmdID < 0 and not interactionState.areBuildDragging then
		...
		if buildIcon then
			...
			local buildFacing = Engine.Unsynced.GetBuildFacing()
			local canBuild = Engine.Shared.TestBuildOrder(buildDefID, wx, wy, wz, buildFacing)

			if canBuild == 2 then
				glFunc.Color(1, 1, 1, 0.5)
			elseif canBuild == 1 then
				local blockedByMobile = false
				local nearbyUnits = Engine.Shared.GetUnitsInCylinder(wx, wz, 64)
				if nearbyUnits then
...
local function DrawBuildDragPreview(iconRadiusZoomDistMult)
	...
	for i = 1, #interactionState.buildDragPositions do
		...
		local canBuild = Engine.Shared.TestBuildOrder(buildDefID, pos.wx, spFunc.GetGroundHeight(pos.wx, pos.wz), pos.wz, buildFacing)
		...
		elseif canBuild == 1 then
			local blockedByMobile = false
			local nearbyUnits = Engine.Shared.GetUnitsInCylinder(pos.wx, pos.wz, 64)
			if nearbyUnits then
				for _, unitID in ipairs(nearbyUnits) do
					local unitDefID = spFunc.GetUnitDefID(unitID)
					if unitDefID and cache.canMove[unitDefID] and not cache.isBuilding[unitDefID] then
...
local function GL4DrawIcons(checkAllyTeamID, selectedSet, trackingSet)
	...
	local resScale = render.contentScale or 1
	local unitBaseSize = Engine.Unsynced.GetConfigFloat("MinimapIconScale", 3.5)
	...
	if not miscState.cachedTakeableTeams then
		...
		if not miscState.isGameOver then
			for _, tID in ipairs(Engine.Shared.GetTeamList()) do
				local _, leader, isDead, hasAI = Engine.Shared.GetTeamInfo(tID, false)
	...
	local bldgBlockWillRebuild = useUnitpics or not gl4Icons._bldgBlock or (Engine.Shared.GetGameFrame() - (gl4Icons._bldgBlockFrame or 0)) >= bldgBlockGameFrameLimit
	local currentGameFrameKeysort = Engine.Shared.GetGameFrame()
	for i = 1, unitCount do
...
local function DrawUnitsAndFeatures(cachedSelectedUnits)
	...
	if interactionState.trackingPlayerID and cameraState.mySpecState then
		...
		miscState.pipUnits = Engine.Shared.GetAllUnits()
	...
	local myAllyTeam = Engine.Unsynced.GetLocalAllyTeamID()
	if interactionState.trackingPlayerID and cameraState.mySpecState then
		local _, _, _, playerTeamID = spFunc.GetPlayerInfo(interactionState.trackingPlayerID, false)
		if playerTeamID then
			local playerAllyTeamID = teamAllyTeamCache[playerTeamID] or Engine.Shared.GetTeamAllyTeamID(playerTeamID)
	...
	elseif not cameraState.mySpecState then
		local myTeamID = Engine.Unsynced.GetLocalTeamID()
		checkAllyTeamID = teamAllyTeamCache[myTeamID] or Engine.Shared.GetTeamAllyTeamID(myTeamID)
	...
	if interactionState.trackingPlayerID then
		...
		if not playerSelections then
			playerSelections = {}
		end -- empty table signals "use tracking mode" to DrawUnit
	end
	...
	if interactionState.trackingPlayerID then
		...
		local selUnits2 = cachedSelectedUnits or Engine.Unsynced.GetSelectedUnits()
	...
	if config.drawComNametags and cameraState.zoom >= config.comNametagZoomThreshold then
		...
		if comNametagCache.dirty then
			...
			for ti = 1, #tList do
				...
				if tID ~= gaiaTeamID and not scavRaptorTeams[tID] then
					local name
					local luaAI = Engine.Shared.GetTeamLuaAI(tID) or ""
					if luaAI ~= "" then
						-- Lua AI team (e.g. Scavengers): use AI display name from game rules
						local aiDisplayName = Engine.Shared.GetGameRulesParam("ainame_" .. tID)
						if aiDisplayName then
							name = aiDisplayName .. " (AI)"
						else
							name = luaAI .. " (AI)"
						end
					elseif Engine.Shared.GetGameRulesParam("ainame_" .. tID) then
						-- Native/C++ AI team (e.g. BARb): no LuaAI, but has ainame_ game rule
						name = Engine.Shared.GetGameRulesParam("ainame_" .. tID) .. " (AI)"
					else
						...
						local players = Engine.Shared.GetPlayerList(tID)
						if players then
							local fallbackName
							for pi = 1, #players do
								local pID = players[pi]
								local pname, active, isspec = Engine.Shared.GetPlayerInfo(pID, false)
								if not isspec then
		...
		local cappedIconRadius = math.min(iconRadiusZoomDistMult, Engine.Unsynced.GetConfigFloat("MinimapIconScale", 3.5) * (mapInfo.mapSizeX * mapInfo.mapSizeZ / 40000) ^ 0.25 * math.sqrt(0.95) * resScale)
	...
	if WG.allycursors and WG.allycursors.getCursor and interactionState.trackingPlayerID then
		...
		if cursor and isNotIdle then
			...
			local _, _, _, teamID = spFunc.GetPlayerInfo(interactionState.trackingPlayerID, false)
			if teamID then
				local r, g, b = Engine.Unsynced.GetTeamColor(teamID)
	...
	if config.showPipTimers and tEnd - perfTimers.lastEchoTime > 2.0 then
		...
		Engine.Shared.Echo(string.format("[PIP%d] total=%.1fms  feat=%.2fms  proj=%.2fms  expl=%.2fms  icons=%.2fms  items=%d", pipNumber, perfTimers.total * 1000, perfTimers.features * 1000, perfTimers.projectiles * 1000, perfTimers.explosions * 1000, perfTimers.icons * 1000, perfTimers.itemCount))
		Engine.Shared.Echo(string.format("  icons: ghost=%.2fms  keysort=%.2fms  sort=%.2fms  process=%.2fms  upload=%.2fms  draw=%.2fms  unitpics=%.2fms  vboReuse=%.0f%%", perfTimers.icGhost * 1000, perfTimers.icKeysort * 1000, perfTimers.icSort * 1000, perfTimers.icProcess * 1000, perfTimers.icUpload * 1000, perfTimers.icDraw * 1000, perfTimers.icUnitpics * 1000, perfTimers.icVboReuse * 100))
		Engine.Shared.Echo(string.format("    process: bldg=%.2fms(%d)  slowMob=%.2fms(%d)  fastMob=%.2fms(%d)", perfTimers.icProcBldg * 1000, perfTimers.icProcBldgN, perfTimers.icProcSlowMobile * 1000, perfTimers.icProcSlowMobileN, perfTimers.icProcMobile * 1000, perfTimers.icProcMobileN))
	end
end
...
local function ShouldShowLOS()
	...
	if interactionState.trackingPlayerID then
		...
		if teamID then
			local allyTeamID = Engine.Shared.GetTeamAllyTeamID(teamID)
...
local function UpdateLavaRenderState()
	...
	local baseLavaLevel = Engine.Shared.GetGameRulesParam("lavaLevel")
	...
	local _, _, isPaused = Engine.Unsynced.GetGameSpeed()
	if not isPaused then
		local camX, camY, camZ = Engine.Unsynced.GetCameraDirection()
		local camvlength = math.sqrt(camX * camX + camZ * camZ + 0.01)
		lrs.smoothFPS = 0.9 * lrs.smoothFPS + 0.1 * math.max(Engine.Unsynced.GetFPS(), 15)
...
local function DrawWaterAndLOSOverlays()
	...
	if mapInfo.hasWater and shaders.water then
		...
		gl.UniformFloat(gl.GetUniformLocation(shaders.water, "gameFrames"), Engine.Shared.GetGameFrame())
	...
	while i <= n do
		...
		if age > 4 or (marker.fadeStart and (currentTime - marker.fadeStart) > 0.4) then
			...
			if filterByAllyTeam and marker.teamID then
				local markerAllyTeam = Engine.Shared.GetTeamAllyTeamID(marker.teamID)
			...
			if shouldDraw then
				...
				if sx >= render.dim.l - edgeMargin and sx <= render.dim.r + edgeMargin and sy >= render.dim.b - edgeMargin and sy <= render.dim.t + edgeMargin then
					...
					elseif marker.teamID then
						r, g, b = Engine.Unsynced.GetTeamColor(marker.teamID)
					end
...
local function DrawBuildCursorWithRotation()
	...
	local buildFacing = Engine.Unsynced.GetBuildFacing()
	local buildTest = Engine.Shared.TestBuildOrder(buildDefID, wx, wy, wz, buildFacing)
...
local function DrawCameraViewBounds()
	...
	local groundY
	local _, centerPos = Engine.Unsynced.TraceScreenRay(math.floor(vsx / 2), math.floor(vsy / 2), true)
	if centerPos then
		groundY = centerPos[2]
	else
	...
	local function screenToGround(sx, sy)
		local dirX, dirY, dirZ = Engine.Unsynced.GetPixelDir(sx, sy)
	...
	local verts = {
		{ bl_c1x, bl_c1y },
		{ bl_c2x, bl_c2y }, -- bottom-left chamfer
		{ br_c1x, br_c1y },
		{ br_c2x, br_c2y }, -- bottom-right chamfer
		{ tr_c1x, tr_c1y },
		{ tr_c2x, tr_c2y }, -- top-right chamfer
		{ tl_c1x, tl_c1y },
		{ tl_c2x, tl_c2y }, -- top-left chamfer
	}
	local nv = #verts
...
local function RenderPipContents()
	...
	if uiState.drawingGround and config.showMapRuler then
		local _, _, spec = spFunc.GetPlayerInfo(Engine.Unsynced.GetLocalPlayerID(), false)
		if not spec then
			BlitMapRuler()
		end
	end
...
local function DrawTrackedPlayerResourceBars()
	...
	local metalCur, metalMax, metalPull, metalIncome, metalExpense, metalShare = Engine.Shared.GetTeamResources(teamID, "metal")
	local energyCur, energyMax, energyPull, energyIncome, energyExpense, energyShare = Engine.Shared.GetTeamResources(teamID, "energy")
	...
	local mmLevel = Engine.Shared.GetTeamRulesParam(teamID, "mmLevel")
	if mmLevel == nil then
		mmLevel = 1
	end

	-- Check if player has teammates (for share slider)
	local _, _, _, _, _, allyTeamID = Engine.Shared.GetTeamInfo(teamID, false)
	local allyTeamList = Engine.Shared.GetTeamList(allyTeamID)
...
local function DrawTrackedPlayerMinimap()
	...
	if interactionState.trackingPlayerID then
		...
		teamID = Engine.Unsynced.GetLocalTeamID()
	end
	if not teamID then
		interactionState.pipMinimapBounds = nil
		return
	...
	if shaders.minimapShading then
		...
		glFunc.BeginEnd(GL.QUADS, function()
			glFunc.TexCoord(0, 0)
			glFunc.Vertex(cLeft, cTop)
			glFunc.TexCoord(1, 0)
			glFunc.Vertex(cRight, cTop)
			glFunc.TexCoord(1, 1)
			glFunc.Vertex(cRight, cBottom)
			glFunc.TexCoord(0, 1)
			glFunc.Vertex(cLeft, cBottom)
		end)
		glFunc.Texture(0, false)
		glFunc.Texture(1, false)
		gl.UseShader(0)
...
local function UpdateR2TUnits(currentTime, pipUpdateInterval, pipWidth, pipHeight)
	...
	local currentRotation = Engine.Unsynced.GetMiniMapRotation and Engine.Unsynced.GetMiniMapRotation() or 0
	...
	local shouldUpdate = pipR2T.unitsNeedsUpdate or rotChanged or driftForced or (sizeChanged and not uiState.areResizing) or pipUpdateInterval == 0 or (pipUpdateInterval > 0 and timeSinceLastUpdate >= pipUpdateInterval)
	...
	if pipR2T.unitsTex then
		...
		gl.R2tHelper.RenderToTexture(pipR2T.unitsTex, function()
			...
			if not ok then
				Engine.Shared.Echo("[PIP] Units render error: " .. tostring(err))
...
local function UpdateR2TCheapLayers(currentTime, pipUpdateInterval, pipWidth, pipHeight)
	...
	if pipR2T.contentTex then
		...
		gl.R2tHelper.RenderToTexture(pipR2T.contentTex, function()
			...
			if not ok then
				Engine.Shared.Echo("[PIP] Cheap layers render error: " .. tostring(err))
...
InitGL4Decals = function()
	...
	local vbo = gl.GetVBO(GL.ARRAY_BUFFER, true)
	if not vbo then
		Engine.Shared.Echo("[PIP] GL4 decals: Failed to create VBO")
		return
	end
	...
	local vao = gl.GetVAO()
	if not vao then
		Engine.Shared.Echo("[PIP] GL4 decals: Failed to create VAO")
		vbo:Delete()
		return
	end
	vao:AttachVertexBuffer(vbo)
	...
	Engine.Shared.Echo("[PIP] GL4 instanced decal rendering enabled")
end
...
local function HandleHoverAndCursor(mx, my)
	...
	if canGiveCommands then
		local _, activeCmdID = Engine.Unsynced.GetActiveCommand()
		if not activeCmdID then
			local defaultCmd = Engine.Unsynced.GetDefaultCommand()
			...
			if not defaultCmd or defaultCmd == 0 then
				if frameSelCount > 0 then
					...
					if lastHoveredUnitID and not Engine.Unsynced.IsUnitAllied(lastHoveredUnitID) then
						local allyTeam = Engine.Shared.GetUnitAllyTeam(lastHoveredUnitID)
						...
						if not isNeutral and isVisibleOrRadar then
							...
							for i = 1, #selectedUnits do
								...
								if uDefID and cache.canAttack[uDefID] then
									Engine.Unsynced.SetMouseCursor("Attack")
									return
								end
							end
						end
					...
					for i = 1, #selectedUnits do
						...
						if uDefID and (cache.canMove[uDefID] or cache.canFly[uDefID]) then
							Engine.Unsynced.SetMouseCursor("Move")
							return
						end
					end
				end
			elseif defaultCmd == CMD.ATTACK and lastHoveredUnitID and not Engine.Unsynced.IsUnitAllied(lastHoveredUnitID) then
			...
			if cursorName then
				Engine.Unsynced.SetMouseCursor(cursorName)
			end
		end
	end
end
...
function widget:DrawScreen()
	...
	local useEngineMinimapFallback = isMinimapMode and config.engineMinimapFallback and #miscState.pipUnits > fallbackUnitThreshold and IsAtMinimumZoom(cameraState.zoom) and IsAtMinimumZoom(cameraState.targetZoom) and not interactionState.trackingPlayerID and not miscState.tvEnabled

	if useEngineMinimapFallback then
		-- Transition: PIP → engine minimap
		if not miscState.engineMinimapActive then
			miscState.baseMinimapIconScale = Engine.Unsynced.GetConfigFloat("MinimapIconScale", 3.5)
			miscState.engineMinimapActive = true
		end
		-- Always update geometry (handles animation, resize, position changes)
		Engine.Unsynced.SendCommands(string.format("minimap geometry %d %d %d %d", math.floor(render.dim.l), math.floor(render.vsy - render.dim.t), math.floor(pipWidth), math.floor(pipHeight)))
		...
		if config.iconDensityScaling and miscState.baseMinimapIconScale then
			...
			local densityScale = 1.0 - (1.0 - config.iconDensityMinScale) * unitFraction
			local resBoost = 1.0 + 0.18 * math.min(math.max((render.vsy - 1080) / (2880 - 1080), 0), 1)
			Engine.Unsynced.SendCommands("minimap unitsize " .. (miscState.baseMinimapIconScale * densityScale * resBoost))
		end
	...
	if gl.R2tHelper and not useEngineMinimapFallback then
		...
		do
			local _, fv = Engine.Unsynced.GetSpectatingState()
...
function widget:Update(dt)
	...
	if cache.guishaderCheckTimer >= 0.5 then
		...
		for tID, cached in pairs(teamColors) do
			local r, g, b, a = Engine.Unsynced.GetTeamColor(tID)
	...
	if state.losViewEnabled and state.losViewAllyTeam then
		local allDead = true
		local teams = Engine.Shared.GetTeamList(state.losViewAllyTeam)
		if teams then
			for t = 1, #teams do
				local _, _, isDead = Engine.Shared.GetTeamInfo(teams[t], false)
				if not isDead then
	...
	if not cameraState.mySpecState then
		cleanupAllyTeam = Engine.Unsynced.GetLocalAllyTeamID()
	elseif state.losViewEnabled and state.losViewAllyTeam then
	...
	if isMinimapMode then
		...
		if cache.leftClickMoveTimer >= 2.0 then
			...
			local cur = Engine.Unsynced.GetConfigInt("MinimapLeftClickMove", 1) == 1
	...
	elseif mapInfo.hasWater then
		...
		local lavaLevel = Engine.Shared.GetGameRulesParam("lavaLevel")
	...
	if isMinimapMode and not miscState.minimapMinimizeAnimating and not uiState.isAnimating then
		local wantMinimized = Engine.Unsynced.GetConfigInt("MinimapMinimize", 0) == 1
	...
	local specState, fullviewState = Engine.Unsynced.GetSpectatingState()
	...
	if specState and fullviewState then
		...
		if now - miscState.specGhostScanTime >= 2.0 then
			...
			local scanAllyTeam = (state.losViewEnabled and state.losViewAllyTeam) or Engine.Unsynced.GetLocalAllyTeamID()
	...
	if interactionState.isMouseOverPip and shouldCheckHover and not isZooming then
		...
		if cmdID and cmdID > 0 then -- Positive cmdID means it's a command (not a build command)
			...
			elseif unitID then
				...
				local isAlly = Engine.Unsynced.IsUnitAllied(unitID)
				local setTargetCmd = GameCMD and GameCMD.UNIT_SET_TARGET_NO_GROUND
		...
		elseif unitID then
			...
			if isAlly then
				...
				local allyTeam = Engine.Shared.GetUnitAllyTeam(unitID)
				local isNeutral = (allyTeam == cache.gaiaAllyTeamID)
	...
	if not gameHasStarted and not isMinimapMode then
		...
		if not isSpec and not interactionState.trackingPlayerID then
			local newX, _, newZ = Engine.Shared.GetTeamStartPosition(Engine.Unsynced.GetLocalTeamID())
	...
	if interactionState.areBoxSelecting then
		...
		if shouldUpdate then
			...
			if WG.SmartSelect_SelectUnits then
				...
				Engine.Unsynced.SelectUnitArray(unitsInBox, shift) -- shift = append mode
			end
		end
	end
end
...
function widget:UnitSeismicPing(x, y, z, strength, allyTeam, unitID, unitDefID)
	...
	local unitAllyTeam = unitID and Engine.Shared.GetUnitAllyTeam(unitID) or allyTeam
...
function widget:CrashingAircraft(unitID, unitDefID, teamID)
	...
	if not uiState.inMinMode then
		local vx, vy, vz = Engine.Shared.GetUnitVelocity(unitID)
...
function widget:UnitDestroyed(unitID, unitDefID, unitTeam)
	...
	miscState.crashingUnits[unitID] = Engine.Shared.GetGameFrame()
	...
	if ghostBuildings[unitID] then
		...
		if state.losViewEnabled and state.losViewAllyTeam then
			local gy = Engine.Shared.GetGroundHeight(ghost.x, ghost.z)
			if Engine.Shared.IsPosInLos(ghost.x, gy, ghost.z, state.losViewAllyTeam) then
...
function widget:UnitCommand(unitID, unitDefID, unitTeam, cmdID, cmdParams, cmdOpts, cmdTag)
	...
	if cameraState.mySpecState then
		...
		if interactionState.trackingPlayerID then
			local _, _, _, playerTeamID = spFunc.GetPlayerInfo(interactionState.trackingPlayerID, false)
			if playerTeamID then
				viewAllyTeam = teamAllyTeamCache[playerTeamID] or Engine.Shared.GetTeamAllyTeamID(playerTeamID)
			end
		elseif state.losViewEnabled and state.losViewAllyTeam then
			...
			local _, fullview = Engine.Unsynced.GetSpectatingState()
			if not fullview then
				viewAllyTeam = Engine.Unsynced.GetLocalAllyTeamID()
			end
		end
...
function widget:UnitFinished(unitID, unitDefID, unitTeam)
	...
	if cameraState.mySpecState and cache.isBuilding[unitDefID] then
		local myAllyTeam = Engine.Unsynced.GetLocalAllyTeamID()
		local uAllyTeam = Engine.Shared.GetTeamAllyTeamID(unitTeam)
		if uAllyTeam ~= myAllyTeam then
			local losBits = Engine.Shared.GetUnitLosState(unitID, myAllyTeam, true)
			if losBits and (losBits % 2 >= 1 or losBits % 8 >= 4) then
				local x, _, z = Engine.Shared.GetUnitBasePosition(unitID)
				if x then
		...
		if cache.isCommander[unitDefID] then
			...
			if hp and maxHP and maxHP > 0 then
				...
				if healthFrac < 0.5 then
					...
					local nearestEnemy = Engine.Shared.GetUnitNearestEnemy(unitID, 600, true)
...
function widget:VisibleExplosion(px, py, pz, weaponID, ownerID)
	...
	local explosion = {
		...
		z = pz,
		radius = radius,
		startFrame = Engine.Shared.GetGameFrame(), -- game-frame based: freezes when paused
	...
	if not isLightning and not isParalyze and not isJuno then
		...
		elseif isUnitExplosion and ownerID then
			local ownerDefID = Engine.Shared.GetUnitDefID(ownerID)
...
function widget:DefaultCommand()
	...
	if mx >= render.dim.l and mx <= render.dim.r and my >= render.dim.b and my <= render.dim.t then
		...
		local uID = GetUnitAtPoint(wx, wz)
		if uID then
			if Engine.Unsynced.IsUnitAllied(uID) then
				return CMD.GUARD
			else
				return CMD.ATTACK
			end
...
function widget:MapDrawCmd(playerID, cmdType, mx, my, mz, a, b, c)
	...
	if cmdType == "point" then
		...
		if showMarker then
			...
			if triggerFocus and playerID == Engine.Unsynced.GetLocalPlayerID() then
				triggerFocus = false
			end
			...
			if triggerFocus and config.activityFocusBlockIgnoredPlayers and WG.ignoredAccounts then
				local pName, _, _, _, _, _, _, _, _, _, pInfo = Engine.Shared.GetPlayerInfo(playerID)
	...
	if screenX >= render.dim.l and screenX <= render.dim.r and screenY >= render.dim.b and screenY <= render.dim.t and not uiState.inMinMode then
		...
		if cmdType == "point" then
			...
			Engine.Unsynced.MarkerAddPoint(wx, markerHeight, wz, c or "")
		elseif cmdType == "line" then
			...
			if interactionState.lastMapDrawX and interactionState.lastMapDrawZ then
				...
				Engine.Unsynced.MarkerAddLine(interactionState.lastMapDrawX, lastY, interactionState.lastMapDrawZ, wx, markerHeight, wz)
			end
		...
		elseif cmdType == "erase" then
			...
			Engine.Unsynced.MarkerErasePosition(wx, wy, wz)
		end
	...
	if not uiState.inMinMode then
		...
		if mx >= render.dim.l and mx <= render.dim.r and my >= render.dim.b and my <= render.dim.t then
			...
			if config.altKeyRequiredForZoom then
				local alt = Engine.Unsynced.GetModKeyState()
...
function widget:MousePress(mx, my, mButton)
	...
	if isMinimapMode and miscState.minimapMinimized then
		if mButton == 1 then
			...
			if mx >= btnL and mx <= btnR and my >= btnB and my <= btnT then
				...
				Engine.Unsynced.SetConfigInt("MinimapMinimize", 0)
	...
	if mButton == 1 and mmBounds and not interactionState.trackingPlayerID and not uiState.inMinMode then
		if mx >= mmBounds.l and mx <= mmBounds.r and my >= mmBounds.b and my <= mmBounds.t then
			...
			local minimapRotation = Engine.Unsynced.GetMiniMapRotation()
			if minimapRotation ~= 0 then
	...
	if uiState.inMinMode then
		...
		if (mButton == 1 or mButton == 2) and mx >= uiState.minModeL and mx <= uiState.minModeL + math.floor(render.usedButtonSize * config.maximizeSizemult) and my >= uiState.minModeB and my <= uiState.minModeB + math.floor(render.usedButtonSize * config.maximizeSizemult) then
			local altKey = Engine.Unsynced.GetModKeyState()
	...
	if mx >= render.dim.l and mx <= render.dim.r and my >= render.dim.b and my <= render.dim.t then
		...
		if mButton == 1 then
			...
			if isMinimapMode and config.minimapModeShowMinimizeButton and mx >= render.dim.r - render.usedButtonSize and my >= render.dim.t - render.usedButtonSize then
				Engine.Unsynced.SetConfigInt("MinimapMinimize", 1)
				Engine.Unsynced.SendCommands("minimap minimize 1")
			...
			if isMinimapMode and IsLeftClickPanActive() and not alt then
				...
				if not cmdID or cmdID == 0 then
					...
					Engine.Unsynced.SetCameraTarget(wx, groundHeight, wz, 0.2)
			...
			if not interactionState.arePanning then
				...
				if not alt and not interactionState.trackingPlayerID then
					if IsLeftClickPanActive() and not interactionState.trackingPlayerID then
						...
						interactionState.selectionBeforeBox = Engine.Unsynced.GetSelectedUnits()
		...
		elseif mButton == 3 then
			...
			local _, activeCmd = Engine.Unsynced.GetActiveCommand()
						...
						if Engine.Unsynced.IsUnitAllied(uID) then
							...
							if canLoadTarget then
								...
								if canGiveCommands then
									...
									Engine.Unsynced.SetActiveCommand(Engine.Unsynced.GetCmdDescIndex(CMD.LOAD_UNITS))
									return true
								end
							else
								cmdID = CMD.GUARD
...
function widget:MouseMove(mx, my, dx, dy, mButton)
	...
	if interactionState.worldCameraDragging then
		...
		local wx, wz = PipToWorldCoords(mx, my)
		local groundHeight = spFunc.GetGroundHeight(wx, wz) or 0
		Engine.Unsynced.SetCameraTarget(wx, groundHeight, wz, 0.04)
		return true
	end
			...
			if math.abs(dx) > 2 or math.abs(dy) > 2 then
				...
				if interactionState.areBoxSelecting then
					...
					if interactionState.selectionBeforeBox then
						Engine.Unsynced.SelectUnitArray(interactionState.selectionBeforeBox)
	...
	elseif interactionState.arePanning then
		...
		if dx ~= 0 or dy ~= 0 then
			...
			Engine.Unsynced.WarpMouse(centerX, centerY)
			-- Reapply the current cursor to prevent it from disappearing after warp
			Engine.Unsynced.SetMouseCursor(Engine.Unsynced.GetMouseCursor())
		end
	elseif interactionState.areBoxSelecting then
		...
		if (currentTime - interactionState.lastBoxSelectUpdate) > 0.033 then
			...
			if WG.SmartSelect_SelectUnits then
				...
				-- Fallback to engine default if smart select is disabled
				local _, ctrl, _, shift = Engine.Unsynced.GetModKeyState()
				if ctrl then
					-- Deselect mode
					local currentSelection = Engine.Unsynced.GetSelectedUnits()
					local newSelection = {}
					...
					Engine.Unsynced.SelectUnitArray(newSelection)
				else
					Engine.Unsynced.SelectUnitArray(unitsInBox, shift) -- shift = append mode
				end
			end
			...
			if uID then
				...
				if ctrl then
					...
					if spFunc.IsUnitSelected(uID) then
						...
						Engine.Unsynced.SelectUnitArray({ uID }, true)
					end
				elseif shift then
					...
					Engine.Unsynced.SelectUnitArray({ uID }, false)
				end
			else
			...
			if interactionState.areBoxDeselecting then
				...
				if WG.SmartSelect_SelectUnits then
					...
					if #unitsInBox > 0 then
						Engine.Unsynced.SelectUnitArray(unitsInBox, shift)
					else
	...
	if mButton == 2 then
		if interactionState.middleMousePressed then
			...
			if wasClick and config.middleClickTeleport then
				...
				local curCamState = Engine.Unsynced.GetCameraState()
				if curCamState then
					-- Set position
					curCamState.px = wx
					curCamState.pz = wz
					...
					Engine.Unsynced.SetCameraState(curCamState, 0.2)
				else
