...
local minHeight, _, _, _ = Engine.Shared.GetGroundExtremes()
...
local SetActiveCommand = Engine.Unsynced.SetActiveCommand
...
local function addUnitDefPair(firstUnitName, lastUnitName)
	...
	if not (firstUnitDef and lastUnitDef and firstUnitDef.id and lastUnitDef.id) then
		Engine.Shared.Echo(string.format("%s: can't add %s/%s pair", "cmd_context_build", firstUnitName, lastUnitName))
		return
	end
