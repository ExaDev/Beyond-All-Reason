...
for i = 1, #teams do
	...
	local allyTeamID = select(6, Engine.Shared.GetTeamInfo(teamID, false))
...
function widget:VisibleUnitsChanged(extVisibleUnits, extNumVisibleUnits)
	...
	for unitID, unitDefID in pairs(extVisibleUnits) do
		AddUnit(unitID, unitDefID, Engine.Shared.GetUnitTeam(unitID), true) -- add them with noUpload = true
	end
...
local function init()
	...
	if WG.unittrackerapi and WG.unittrackerapi.visibleUnits then
		...
		Engine.Shared.Echo("Enemy spotter needs unittrackerapi to work!")
		widgetHandler:RemoveWidget()
		return false
	end
	return true
