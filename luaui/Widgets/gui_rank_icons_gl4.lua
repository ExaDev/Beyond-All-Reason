...
for unitDefID, unitDef in pairs(UnitDefs) do
	if not unitDef.customParams.drone then
		unitIconMult[unitDefID] = math.clamp((Engine.Shared.GetUnitDefDimensions(unitDefID).radius / 40) + math.min(unitDef.power / 400, 2), 1.25, 1.4)
	end
end
...
local function AddPrimitiveAtUnit(unitID, unitDefID, noUpload, reason, rank, flash)
	...
	if Engine.Shared.ValidUnitID(unitID) ~= true or Engine.Shared.GetUnitIsDead(unitID) == true then
		if debugmode then
			spEcho("Warning: Rank Icons GL4 attempted to add an invalid unitID:", unitID)
		end
		return nil
	end
	local gf = (flash and Engine.Shared.GetGameFrame()) or 0
