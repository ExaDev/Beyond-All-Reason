...
do
	local validStartUnits = string.split(Engine.Shared.GetTeamRulesParam(myTeamID, "validStartUnits") or Engine.Shared.GetGameRulesParam("validStartUnits"), "|")
...
if #factions == 0 then
	Engine.Shared.Log(gadget:GetInfo().name, LOG.ERROR, "No Start Options Recived")
	return false
end
...
local function drawFactionpicker()
	...
	for i, faction in pairs(factions) do
		...
		local disabled = Engine.Shared.GetTeamRulesParam(myTeamID, "startUnit") ~= factions[i].startUnit
		if disabled then
...
function widget:Initialize()
	if isSpec or Engine.Shared.GetGameFrame() > 0 then
		widgetHandler:RemoveWidget()
		return
	end

	if Engine.Shared.GetModOptions().scenariooptions then
		local scenarioopts = string.base64Decode(Engine.Shared.GetModOptions().scenariooptions)
...
function widget:DrawScreen()
	local x, y, b = Engine.Unsynced.GetMouseState()
...
function widget:MousePress(x, y, button)
	if math_isInRect(x, y, backgroundRect[1], backgroundRect[2], backgroundRect[3], backgroundRect[4]) then
		for i, faction in pairs(factions) do
			if math_isInRect(x, y, factionRect[i][1], factionRect[i][2], factionRect[i][3], factionRect[i][4]) then
				if playSounds then
					Engine.Unsynced.PlaySoundFile(sound_button, 0.6, "ui")
				end
				-- tell initial spawn
				Engine.Unsynced.SendLuaRulesMsg("changeStartUnit" .. tostring(factions[i].startUnit))
				break
			end
		end
		return true
