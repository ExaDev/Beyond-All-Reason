...
local function AddPrimitiveAtUnit(featureID, featureDefID, noUpload)
	...
	featureDefID = featureDefID or Engine.Shared.GetFeatureDefID(featureID)
...
function widget:DrawWorldPreUnit()
	...
	if groundPlateVBO.usedElements > 0 then
		local disticon = Engine.Unsynced.GetConfigInt("FeatureFadeDistance", 200) -- iconLength = unitIconDist * unitIconDist * 750.0f;
		glCulling(GL_BACK)
		...
		if FD.modelname and (string.find(FD.modelname:lower(), "_dead", nil, true) or string.find(FD.name, "_heap", nil, true)) then -- todo TREES!
			...
			if atlassedImages[modelnamenos3o] then
				...
				if string.find(atlasname, "_dead", nil, true) then -- regular wrecks
					sizestr = string.sub(atlasname, (string.find(atlasname, "_dead", nil, true) or 0) + 6, string.find(atlasname, "_aoplane.dds", nil, true) - 1)
				else -- heaps override
...
local allyTeamID = Engine.Unsynced.GetLocalAllyTeamID()

function widget:PlayerChanged()
	local prevFullview = fullview
	local myPrevAllyTeamID = allyTeamID
	spec, fullview = spGetSpectatingState()
	allyTeamID = Engine.Unsynced.GetLocalAllyTeamID()
end
			...
			if coords then
				for z = coords[3], coords[3] + size * maxx, size do
					for x = coords[1], coords[1] + size * maxx, size do
						if i <= #matches then
							commandqueue[#commandqueue + 1] = string.format("give %s @%d,%d,%d", matches[i], x, Engine.Shared.GetGroundHeight(x, z), z)
							i = i + 1
						end
					end
				end
...
function widget:GameFrame()
	if #commandqueue > 0 then
		Engine.Unsynced.SendCommands({ commandqueue[#commandqueue] })
