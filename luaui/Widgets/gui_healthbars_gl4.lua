...
local GetUnitWeaponState = Engine.Shared.GetUnitWeaponState
...
for udefID, unitDef in pairs(UnitDefs) do
	...
	unitDefSizeMultipliers[udefID] = mathMin(1.45, math.max(0.85, (Engine.Shared.GetUnitDefDimensions(udefID).radius / 150) + mathMin(0.6, unitDef.power / 4000))) + mathMin(0.6, unitDef.health / 22000)
...
local function addBarForUnit(unitID, unitDefID, barname, reason)
	...
	if unitDefID == nil or Engine.Shared.ValidUnitID(unitID) == false or Engine.Shared.GetUnitIsDead(unitID) == true then -- dead or invalid
		if debugmode then
	...
	if unitBars[unitID] == nil then
		if debugmode then
			...
			Engine.Unsynced.MarkerAddPoint(spGetUnitPosition(unitID))
		end
		unitBars[unitID] = 1
	end
...
local function addBarsForUnit(unitID, unitDefID, unitTeam, unitAllyTeam, reason) -- TODO, actually, we need to check for all of these for stuff entering LOS
	if unitDefID == nil or Engine.Shared.ValidUnitID(unitID) == false or Engine.Shared.GetUnitIsDead(unitID) == true then
		if debugmode then
	...
	unitAllyTeam = unitAllyTeam or Engine.Shared.GetUnitAllyTeam(unitID)
	...
	if (fullview or (unitAllyTeam == myAllyTeamID) or (unitDefHideDamage[unitDefID] == nil)) and (unitDefIgnore[unitDefID] == nil) then
		if debugmode and health == nil then
			...
			Engine.Unsynced.MarkerAddPoint(ux, uy, uz, "health")
		end
	...
	elseif unitDefReactiveArmor[unitDefID] then
		...
		addBarForUnit(unitID, unitDefID, "shield", reason)
		local armorHealth = Engine.Shared.GetUnitRulesParam(unitID, "reactiveArmorHealth")
		if armorHealth then
...
local function init()
	...
	unitReloadWatch = {}
	unitBars = {}
	for i, unitID in ipairs(Engine.Shared.GetAllUnits()) do -- gets radar blips too!
		...
		if fullview then
			...
			local losstate = Engine.Shared.GetUnitLosState(unitID, myAllyTeamID)
...
local function UnitParalyzeDamageHealthbars(unitID, unitDefID, damage)
	if Engine.Shared.GetUnitIsStunned(unitID) then -- DO NOTE THAT: return: nil | bool stunned_or_inbuild, bool stunned, bool inbuild
		if unitParalyzedWatch[unitID] == nil then -- already paralyzed
			unitParalyzedWatch[unitID] = 0.0
			-- if unit was already empd, remove that bar
			if unitEmpDamagedWatch[unitID] then
...
function widget:PlayerChanged(playerID)
	...
	local currentTeamID = Engine.Unsynced.GetLocalTeamID()
...
function widget:GameFrame(n)
	...
	if n % 3 == 0 then
		for unitID, oldshieldPower in pairs(unitShieldWatch) do
			local shieldOn, shieldPower = Engine.Shared.GetUnitShieldState(unitID)
		...
		for unitID, oldArmorValue in pairs(unitReactiveArmorWatch) do
			local newArmorValue = Engine.Shared.GetUnitRulesParam(unitID, "reactiveArmorHealth")
	...
	if (n % 5) == 2 then
		for unitID, stockpilebuild in pairs(unitStockPileWatch) do
			local numStockpiled, numStockpileQued, stockpileBuild = Engine.Shared.GetUnitStockpile(unitID)
...
function widget:FeatureCreated(featureID)
	...
	if FeatureDefs[featureDefID].name ~= "geovent" and FeatureDefs[featureDefID].modelpath ~= "" then
		...
		local health, maxhealth, rezProgress = Engine.Shared.GetFeatureHealth(featureID)
		...
		local _, _, _, _, reclaimLeft = Engine.Shared.GetFeatureResources(featureID)
...
function widget:DrawScreenEffects()
	...
	if not drawWhenGuiHidden and Engine.Unsynced.IsGUIHidden() then
		return
	end
