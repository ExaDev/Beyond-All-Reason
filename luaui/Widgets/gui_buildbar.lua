...
local DrawUnitCommands = Engine.Unsynced.DrawUnitCommands
local GetFullBuildQueue = Engine.Shared.GetFullBuildQueue
local GetUnitIsBuilding = Engine.Shared.GetUnitIsBuilding
...
function widget:Initialize()
	...
	if not gl.R2tHelper then
		Engine.Shared.Echo("BuildBar: gl.R2tHelper not available")
		widgetHandler:RemoveWidget()
		return
	end
	...
	if WG.info then
		if hoveredFac >= 0 then
			if not setInfoDisplayUnitID or (hoveredBOpt < 0 and setInfoDisplayUnitID ~= facs[hoveredFac + 1].unitID) then
				Engine.Unsynced.PlaySoundFile(sound_hover, 0.8, "ui")
...
function widget:DrawInMiniMap(sx, sy)
	if openedMenu > -1 then
		...
		if anonymousMode ~= "disabled" then
			...
			r, g, b = Engine.Unsynced.GetTeamColor(myTeamID)
		end
		local alpha = 0.5 + mathAbs((Engine.Shared.GetGameSeconds() % 0.25) * 4 - 0.5)
		local x, _, z = Engine.Shared.GetUnitBasePosition(facs[openedMenu + 1].unitID)
...
local function menuHandler(x, y, button)
	...
	if button == 1 then
		...
		if isInRect(x, y, icoRect) then
			...
			Engine.Unsynced.PlaySoundFile(sound_click, 0.8, "ui")
		else
			Engine.Unsynced.SelectUnitArray({ factoryUnitID })
		end
	elseif button == 3 then
		Engine.Unsynced.SelectUnitArray({ factoryUnitID })
		Engine.Unsynced.SetCameraTarget(Engine.Shared.GetUnitPosition(factoryUnitID))
	end
