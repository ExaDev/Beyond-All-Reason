...
function widget:Initialize()
	...
	myAllyTeam = Engine.Unsynced.GetLocalAllyTeamID()
...
local function UnitDetected(unitID, unitDefID, unitTeam, noUpload)
	...
	--local weapons = unitWeapons[unitDefID]
	local alliedUnit = (Engine.Shared.GetUnitAllyTeam(unitID) == myAllyTeam)
	...
	for i, weaponType in pairs(unitDefRings[unitDefID].weapons) do
		...
		if buttonConfig[allystring][weaponType] or (colorConfig.drawAllyCategoryBuildQueue and (allystring == "ally")) then
			...
			local x, y, z, mpx, mpy, mpz, apx, apy, apz = spGetUnitPosition(unitID, true, true)
			local wpx, wpy, wpz, wdx, wdy, wdz = Engine.Shared.GetUnitWeaponVectors(unitID, weaponID)
...
function widget:VisibleUnitsChanged(extVisibleUnits, extNumVisibleUnits)
	...
	-- my
	spec, fullview = Engine.Unsynced.GetSpectatingState()
...
local function checkEnemyUnitConfirmedDead(unitID, defense)
	...
	if losState then -- visible
		if Engine.Shared.GetUnitIsDead(unitID) ~= false then -- If its cloaked and jammed, we cant see it i think
			return true
		end
	end
	return false
...
function widget:PlayerChanged(playerID)
	...
	local nowmyAllyTeam = Engine.Unsynced.GetLocalAllyTeamID()
...
function widget:Update(dt)
	...
	local currentAllyTeam = Engine.Unsynced.GetLocalAllyTeamID()
	if currentAllyTeam ~= myAllyTeam then
		...
		local extVisibleUnits = (WG.unittrackerapi and WG.unittrackerapi.visibleUnits) or (function()
			local t = {}
			for _, uid in ipairs(Engine.Shared.GetAllUnits()) do
	...
	local cmdID = select(2, Engine.Unsynced.GetActiveCommand())
	...
	if cmdID ~= nil and (cmdID < 0) then
		...
		if unitDefRings[buildUnitDefID] then
			...
			if coords and coords[1] and coords[2] and coords[3] then
				local bpx, bpy, bpz = Engine.Shared.Pos2BuildPos(buildUnitDefID, coords[1], coords[2], coords[3])
				local allystring = "ally"
...
local function GetCameraHeightFactor() -- returns a smoothstepped value between 0 and 1 for height based rescaling of line width.
	...
	local camheight = camY - mathMax(Engine.Shared.GetGroundHeight(camX, camZ), 0)
...
function widget:DrawWorld()
	...
	if not Engine.Unsynced.IsGUIHidden() and (not WG.topbar or not WG.topbar.showingQuit()) then
		...
		glTexture(1, false)
		glDepthTest(false)
		if false and Engine.Unsynced.GetDrawFrame() % 60 == 0 then
			local s = "drawcounts: "
