local widget = widget ---@type Widget

function widget:GetInfo()
	return {
		name = "Death Messages",
		desc = "Displays a message upon player/team death",
		author = "Bluestone",
		date = "Sept 2013",
		license = "GNU GPL, v2 or later, BA/BAR only",
		layer = 0,
		enabled = true,
	}
end

-- Localized functions for performance
local tableInsert = table.insert

local deathMessageKeys = {
	"bowOut",
	"gone",
	"conquer",
	"toast",
	"takenOut",
	"defeat",
	"bitterEnd",
	"rodeOff",
	"dismantle",
	"terminate",
	"annihilate",
	"crater",
}
local teamNames = {}

local function getTeamNames(teamID)
	local playerNames = {}
	local _, _, _, isAI = Engine.Shared.GetTeamInfo(teamID, false)

	if isAI then
		local _, _, _, name = Engine.Shared.GetAIInfo(teamID)
		local niceName = Engine.Shared.GetGameRulesParam("ainame_" .. teamID)

		if niceName then
			name = niceName
		end

		tableInsert(playerNames, name)
	else
		local players = Engine.Shared.GetPlayerList(teamID)

		for _, playerID in pairs(players) do
			local name = Engine.Shared.GetPlayerInfo(playerID, false)
			name = ((WG.playernames and WG.playernames.getPlayername) and WG.playernames.getPlayername(playerID)) or name
			tableInsert(playerNames, name)
		end
	end

	return playerNames
end

local function notifyTeamDeath(teamID)
	local playerNameList = teamNames[teamID]

	if playerNameList == nil or next(playerNameList) == nil then
		Engine.Shared.Log(widget:GetInfo().name, LOG.ERROR, "Team " .. teamID .. ": no names in players list")
	else
		local playerNames = table.concat(playerNameList, ", ")
		local n = math.random(#deathMessageKeys)
		local message = BAR.I18N("tips.deathMessages.team." .. deathMessageKeys[n], { playerList = playerNames })

		Engine.Unsynced.SendMessage(message)
	end
end

function widget:TeamDied(teamID)
	notifyTeamDeath(teamID)
end

function widget:Initialize()
	local teams = Engine.Shared.GetTeamList()

	for _, teamID in pairs(teams) do
		teamNames[teamID] = getTeamNames(teamID)
	end
end
