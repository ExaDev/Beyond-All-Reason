...
local sp = {
	...
	GetMyAllyTeamID = Engine.Unsynced.GetLocalAllyTeamID,
...
local function drawUi()
	...
	if windFunctions.isNoWind() then
		...
		font2:Print("\255\200\200\200" .. BAR.I18N("ui.topbar.wind.nowind1"), windSkewCX, windArea[2] + ((windArea[4] - windArea[2]) / 1.5) - (fontsize / 5), fontsize * 1.06, "oc") -- Wind speed text
		font2:Print("\255\200\200\200" .. BAR.I18N("ui.topbar.wind.nowind2"), windSkewCX, windArea[2] + ((windArea[4] - windArea[2]) / 2.8) - (fontsize / 5), fontsize * 1.06, "oc") -- Wind speed text
		font2:End()
	end
...
function widget:DrawScreen()
	...
	if not windFunctions.isNoWind() then
		if currentWind ~= prevWind or refreshUi then
			...
			r2tHelper.RenderToTexture(uiTex, renderWindText, true, { windArea[1] - topbarArea[1], windArea[2] - topbarArea[2], windArea[3] - windArea[1], windArea[4] - windArea[2] })
		end
	end
	...
	if displayComCounter and dlist.coms then
		...
		if comsDlistUpdate or prevComAlert == nil or (prevComAlert ~= (allyComs == 1 and (gameFrame % 12 < 6))) then
			...
			r2tHelper.RenderToTexture(uiTex, renderComCounter, true, { comsArea[1] - topbarArea[1], 0, comsArea[3] - comsArea[1], (topbarArea[4] - topbarArea[2]) })
		end
	end
...
function widget:Initialize()
	...
	if UnitDefs[Engine.Shared.GetTeamRulesParam(Engine.Unsynced.GetLocalTeamID(), "startUnit")] then
		textures.com = ":n:Icons/" .. UnitDefs[Engine.Shared.GetTeamRulesParam(Engine.Unsynced.GetLocalTeamID(), "startUnit")].name .. ".png"
	end
