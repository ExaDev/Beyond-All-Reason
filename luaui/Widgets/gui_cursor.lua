...
Settings.sizeMult = Engine.Unsynced.GetConfigFloat("cursorsize", 1)
...
function widget:ViewResize()
	local ssx, ssy = Engine.Unsynced.GetScreenGeometry() -- doesnt change when you unplug external display
	autoCursorSize = 100 * (0.6 + (ssx * ssy / 10000000)) * Engine.Unsynced.GetConfigFloat("cursorsize", 1)
...
function widget:Initialize()
	...
	WG.cursors.getsizemult = function()
		return Engine.Unsynced.GetConfigFloat("cursorsize", 1)
	end
	WG.cursors.setsizemult = function(value)
		Engine.Unsynced.SetConfigFloat("cursorsize", value)
		widget:ViewResize()
	end
end
...
function widget:Shutdown()
	...
	if file then
		Engine.Unsynced.LoadCmdColorsConfig(file)
	end
end
...
function SetCursor(cursorSet)
	...
	if cursorDir ~= oldSetName or force then
		...
		for i = 1, #cursorNames do
			Engine.Unsynced.ReplaceMouseCursor(cursorNames[i], cursorDir .. "/" .. cursorNames[i], (cursorNames[i] == "cursornormal"))
		end
		...
		if WG.resource_spot_finder and not WG.resource_spot_finder.isMetalMap then
			Engine.Unsynced.LoadCmdColorsConfig("rangeExtract         1.0  0.3  0.3  0.0")
		end
	end
end
...
function widget:SetConfigData(data)
	if data and type(data) == "table" and data.version then
		if data.version < 6 and data.sizeMult then
			Engine.Unsynced.SetConfigFloat("cursorsize", data.sizeMult)
		end
		Settings = data
		Settings.sizeMult = Engine.Unsynced.GetConfigFloat("cursorsize", 1)
	end
