...
local function MakeAtlasOnDemand(config)
	...
	function AtlasOnDemand:ReserveSpace(id, xsize, ysize)
		if self.uvcoords[id] then
			Engine.Shared.Echo(string.format("AtlasOnDemand %s Warning: ID %s is already added to this atlas", self.name, tostring(id)))
		...
		if foundspot then
			...
			for x = self.firstemptycolumn, self.firstemptycolumn + xstep - 1 do
				...
				if columnfull then
					if self.debug then
						Engine.Shared.Echo("Column Full!", self.firstemptycolumn)
					end
			...
			Engine.Shared.Echo(string.format("AtlasOnDemand %s Error: cant find space for %s of size %d x %d", self.name, tostring(id), xsize, ysize))
	...
	function AtlasOnDemand:AddImage(image, xsize, ysize)
		...
		if self.uvcoords[image] then
			Engine.Shared.Echo(string.format("AtlasOnDemand %s Warning: image %s is already added to this atlas", self.name, tostring(image)))
		...
		if xsize and ysize then
			...
			if texInfo and (texInfo.xsize ~= xsize or texInfo.ysize ~= ysize) then
				Engine.Shared.Echo(string.format("AtlasOnDemand %s Warning: image %s size does not match %dx%d given, %dx%d from TextureInfo", self.name, tostring(image), xsize or -1, ysize or -1, texInfo.xsize, texInfo.ysize))
				if xsize == nil then
					xsize = texInfo.xsize
				end
				if ysize == nil then
	...
	function AtlasOnDemand:PlaceImage(image, x, y, xsize, ysize)
		...
		if texInfo then
			...
			Engine.Shared.Echo(string.format("AtlasOnDemand %s Warning:   unable to read gl.TextureInfo(%s)", self.name, tostring(image)))
		end
	end
	...
	function AtlasOnDemand:AddText(text, params)
		...
		if not params then -- render with default fot
			if self.uvcoords[text] then
				...
				if not self.defaultfont then
					Engine.Shared.Echo(string.format("AtlasOnDemand %s Warning: text %s without font cannot be added because atlas has no default font set", self.name, tostring(id)))
		...
		if self.uvcoords[textparams.id] then
			Engine.Shared.Echo(string.format("AtlasOnDemand %s Warning: text %s is already added to this atlas", self.name, tostring(textparams.id)))
		...
		if self.debug then
			Engine.Shared.Echo(string.format("Pre adjusted textsize for %s at size %d is w=%d h=%d descender=%d", textparams.text, textparams.size, width, height, textdescender))
		end
		local pad = 1
		...
		if self.debug then
			Engine.Shared.Echo(string.format('AddText: "%s", size=%d, w=%d h=%d d=%d pad =%f', textparams.text, textparams.size, width, height, textdescender, pad))
		end
	...
	function AtlasOnDemand:RenderTextTasks()
		...
		for i, task in ipairs(taskList) do
			...
			if currentParams.font ~= textparams.font then
				...
				if self.debug then
					Engine.Shared.Echo("Font set to", textparams.font.path, textparams.size)
				end
			...
			if self.debug then
				Engine.Shared.Echo(string.format("Task params: id = %s w=%.3f h=%.3f x=%.3f y=%.3f", task.id, task.w, task.h, task.x, task.y))
				Engine.Shared.Echo(string.format("renderText: text = %s, x = %f, y = %.3f, size = %.3f, opts = %s", textparams.text, x, y, textparams.size, textparams.options))
			end
	...
	function AtlasOnDemand:TextRect(id, x, y, align)
		...
		if not uvcoords then
			Engine.Shared.Echo("AtlasOnDemand:TextRect cannot find id", id)
			return
		end
		local ypos = y
		local xpos = x
	...
	function AtlasOnDemand:PrintAtlas()
		for x = 1, self.xslots do
			...
			Engine.Shared.Echo(table.concat(parts))
		end
	end
	...
	function AtlasOnDemand:DrawToScreen(aliastest, noalpha)
		...
		if aliastest then
			...
			Engine.Shared.Echo(aliasUV.x, aliasUV.y, aliasUV.X, aliasUV.Y, aliasUV.w, aliasUV.h)
			local xs = 0
			for i = 1, 6 do
