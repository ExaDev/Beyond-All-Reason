...
local function echoChunked(lines, chunkSize)
	...
	for i = 1, numLines, chunkSize do
		...
		Engine.Shared.Echo(table.concat(chunkLines, "\n"))
	end
end

function REPORTS.SetDependencies(subLogic)
	Engine.Shared.Log("BlueprintReports", "info", "Receiving dependencies via SetDependencies...")
	substitutionLogic = subLogic
	if not substitutionLogic then
		Engine.Shared.Log("BlueprintReports", "error", "Failed to receive necessary logic tables via SetDependencies!")
	else
		Engine.Shared.Log("BlueprintReports", "info", "Dependencies received successfully via SetDependencies.")
	end
end
	...
	for unitNameLower, buildingCoreData in pairs(masterBuildingDataMinimal) do
		...
		if unitDef then
			...
			local reportLine = table.concat({
				...
				escapeCsvField(translatedHumanName),
				escapeCsvField(side or "???"),
				escapeCsvField(tier),
				escapeCsvField(isBuilding),
				escapeCsvField(name),
				metalCost,
				energyCost,
				buildTime,
				health,
				speed,
				escapeCsvField(hasWeapons),
				escapeCsvField(hasBuildOptions),
				extractsMetal,
				energyMake,
				radarDist,
				sonarDist,
				...
				escapeCsvField(equivArm),
				escapeCsvField(equivCor),
				escapeCsvField(equivLeg),
			}, ",")
			...
			Engine.Shared.Log("BlueprintReports", "warning", "[Mapping Report] Could not find UnitDef for ID: " .. tostring(buildingCoreData.unitDefID) .. " Name: " .. unitNameLower)
		end
	end
...
function REPORTS.generateCategoryListReport()
	...
	Engine.Shared.Log("BlueprintReports", "info", "[Category List] " .. header)
