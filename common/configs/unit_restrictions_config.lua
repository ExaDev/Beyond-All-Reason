...
local function shouldShowWaterUnits()
	...
	local debugCommands = Engine.Shared.GetModOption("debugcommands")
	...
	local moddedWaterLevel = Engine.Shared.GetModOption("map_waterlevel") or 0
...
local function hasGeothermalFeatures()
	...
	for i = 1, #features do
		...
		if geoFeatureDefs[Engine.Shared.GetFeatureDefID(featureID)] then
			local _, y, _ = Engine.Shared.GetFeaturePosition(featureID)
			if y < 0 then
				hasSeaGeo = true
			else
				hasLandGeo = true
