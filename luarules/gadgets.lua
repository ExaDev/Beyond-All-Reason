...
if IsSyncedCode() then
	...
	Engine.Synced.SetGameRulesParam("isDevMode", devModeEnabled)
end
...
function gadgetHandler:LoadGadget(filename, overridevfsmode)
	...
	if gadget.GetInfo and (Platform and not Platform.check(gadget.GetInfo().depends)) then
		Engine.Shared.Echo("Missing capabilities:  " .. gadget:GetInfo().name .. ". Disabling.")
		return nil
	end
	...
	if kbytes then
		...
		collectgarbage("collect") -- sweep
		Engine.Shared.Echo("LoadGadget", filename, "delta=", collectgarbage("count") - kbytes, "total=", collectgarbage("count"), "KB, synced =", IsSyncedCode())
	end
	return gadget
end
...
local function SafeWrap(func, funcName)
	...
	return function(g, ...)
		...
		if ok then
			...
			Engine.Shared.Log(LOG_SECTION, LOG.INFO, r1)
...
function gadgetHandler:InsertGadgetRaw(gadget)
	...
	if gadget.AllowCommand and not self:HasAllowCommands(gadget) then
		Engine.Shared.Log("AllowCommand", LOG.WARNING, "<" .. gadget.ghInfo.basename .. "> AllowCommand defined but didn't register any commands. Autoregistering for all commands!")
	...
	if kbytes then
		collectgarbage("collect")
		collectgarbage("collect")
		Engine.Shared.Echo("Initialize", gadget.ghInfo.name, "delta=", collectgarbage("count") - kbytes, "total=", collectgarbage("count"), "KB, synced =", IsSyncedCode())
	end
end
...
function gadgetHandler:RegisterCMDID(gadget, id)
	if id <= 1000 then
		Engine.Shared.Log(LOG_SECTION, LOG.ERROR, "Gadget (" .. gadget.ghInfo.name .. ") " .. "tried to register a reserved CMD_ID")
		Script.Kill("Reserved CMD_ID code: " .. id)
	end

	if self.CMDIDs[id] ~= nil then
		Engine.Shared.Log(LOG_SECTION, LOG.ERROR, "Gadget (" .. gadget.ghInfo.name .. ") " .. "tried to register a duplicated CMD_ID")
...
function gadgetHandler:RegisterAllowCommand(gadget, cmdID)
	...
	Engine.Shared.Log("AllowCommand", LOG.INFO, "<" .. gadget.ghInfo.basename .. "> Register " .. tostring(cmdID))
	if cmdID == nil then
		-- use CMD.NIL instead
		Engine.Shared.Log("AllowCommand", LOG.ERROR, "<" .. gadget.ghInfo.basename .. "> Invalid cmdID " .. tostring(cmdID))
		return
	end
	if not gadget.AllowCommand then
		Engine.Shared.Log("AllowCommand", LOG.ERROR, "<" .. gadget.ghInfo.basename .. "> No callin method")
		return
	end
