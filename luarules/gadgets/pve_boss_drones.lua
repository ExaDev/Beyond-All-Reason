if not (BAR.Utilities.Gametype.IsRaptors() or BAR.Utilities.Gametype.IsScavengers()) then
	Engine.Shared.Echo("REMOVED PVE BOSS DRONES")
	return false
end
...
local raptorQueenCount = Engine.Shared.GetModOptions().raptor_queen_count
	...
	if unitList[unitDefID] and (unitTeam == pveTeamID) then
		...
		for i = 1, #unitList[unitDefID] do
			aliveCarriers[unitID][i] = {
				aliveDrones = 0,
				lastSpawned = Engine.Shared.GetGameSeconds(),
			}
		end
	end
end
...
function gadget:GameFrame(frame)
	if frame % 30 == 13 then
		for unitID, unitDroneStats in pairs(aliveCarriers) do
			...
			for index, stats in pairs(unitDroneStats) do
				if stats.aliveDrones <= (unitList[unitDefID][index].maxAllowed - unitList[unitDefID][index].spawnedPerWave) and Engine.Shared.GetGameSeconds() >= stats.lastSpawned + unitList[unitDefID][index].spawnTimer then
					for _ = 1, unitList[unitDefID][index].spawnedPerWave do
						...
						if (unitList[unitDefID][index].type == "air" and UnitDefNames[unitList[unitDefID][index].name].canFly) or (unitList[unitDefID][index].type == "ground" and positionCheckLibrary.FlatAreaCheck(spawnx, spawny, spawnz, 64, 30, true)) or (unitList[unitDefID][index].type == "land" and positionCheckLibrary.FlatAreaCheck(spawnx, spawny, spawnz, 64, 30, true) and Engine.Shared.GetGroundHeight(spawnx, spawnz) > 0) or (unitList[unitDefID][index].type == "sea" and positionCheckLibrary.FlatAreaCheck(spawnx, spawny, spawnz, 64, 30, true) and Engine.Shared.GetGroundHeight(spawnx, spawnz) <= 0) then
							...
							if droneID then
								...
								aliveCarriers[unitID][index].lastSpawned = Engine.Shared.GetGameSeconds()
								Engine.Shared.GiveOrderToUnit(droneID, 37382, { 1 }, 0)
	...
	if frame % 30 == 14 then
		for droneID, stats in pairs(aliveDrones) do
			if stats.owner then
				local x, y, z = Engine.Shared.GetUnitPosition(stats.owner)
				if math.random(0, 4) == 0 then
					Engine.Shared.GiveOrderToUnit(droneID, CMD.PATROL, { x + math.random(-stats.fightRadius, stats.fightRadius), y, z + math.random(-stats.fightRadius, stats.fightRadius) }, { "shift" })
				elseif math.random(0, 6) == 0 then
					Engine.Shared.GiveOrderToUnit(droneID, CMD.PATROL, { x + math.random(-stats.fightRadius, stats.fightRadius), y, z + math.random(-stats.fightRadius, stats.fightRadius) }, {})
				end
			else
				if math.random(0, 10) == 0 then
					Engine.Shared.GiveOrderToUnit(droneID, CMD.PATROL, { math.random(0, Game.mapSizeX), 0, math.random(0, Game.mapSizeZ) }, { "shift" })
				end
			end
		end
	end
