...
for weaponID, wd in pairs(WeaponDefs) do
	if wd.type == "Flame" then
		...
		local expectedLife = velPerFrame > 0 and mathMax(8, mathFloor(origRange / velPerFrame)) or CONFIG.expectedLifeFallback
...
local function emitStream(proID, info, gameFrame, throttleMult)
	...
	if lifeT < K.JET_MAX_LIFE_FRAC and budgetTier < 3 then
		...
		if jetCount > 0 then
			...
			for i = 1, jetCount do
				...
				spawnParticle(epx + ox, epy + oy, epz + oz, vx * K.JET_VEL_MULT, vy * K.JET_VEL_MULT, vz * K.JET_VEL_MULT, size, 3, life, jetR, jetG, jetB, jetAlpha)
			end
		end -- if jetCount > 0
	end
	...
	if budgetTier < 1 and not farMode and mathRandom() < K.TAIL_EMIT_CHANCE * burstMult * muzzleTaper then
		...
		spawnParticle(epx + ox, epy + oy, epz + oz, pvx * 0.5 + (mathRandom() * 2 - 1) * pushScale, pvy * 0.5 + (mathRandom() * 2 - 1) * pushScale, pvz * 0.5 + (mathRandom() * 2 - 1) * pushScale, size, 0, life, pR, pG, pB, K.CORE_ALPHA_BASE)
	end
