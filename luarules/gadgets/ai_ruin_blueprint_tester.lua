...
local enabled = BAR.Utilities.IsDevMode() -- only enable in test environment
local queue = {}
...
if gadgetHandler:IsSyncedCode() then
	function gadget:RecvLuaMsg(msg, playerID)
		if string.find(msg, cmdname) then
			local filename = string.gsub(msg, cmdname .. " ", "")
			...
			if #blueprints > 0 then
				...
				for i = 1, #blueprints do
					if i == 1 then
						blueprintpositions[1] = { posx = high_radius * 3, posz = high_radius * 3 }
						lastposX = high_radius * 3
						lastposZ = high_radius * 3
					else
						if lastposX + high_radius * 3 > mapsizeX then
							line = line + 1
							lastposX = 0
							lastposZ = high_radius * line * 3
						end
						blueprintpositions[i] = { posx = lastposX + high_radius * 3, posz = lastposZ }
