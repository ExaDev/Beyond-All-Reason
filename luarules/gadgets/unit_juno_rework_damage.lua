...
function gadget:GetInfo()
	return {
		...
		layer = 0,
		enabled = Engine.Shared.GetModOptions().junorework,
	}
end
...
if gadgetHandler:IsSyncedCode() then
	...
	local stunDuration = Engine.Shared.GetModOptions().emprework and 32 or 30
	...
	function gadget:UnitDamaged(uID, uDefID, uTeam, damage, paralyzer, weaponID, projID, aID, aDefID, aTeam)
		...
		if junoWeapons[weaponID] and toStunUnits[uDefID] and aID ~= 99 and (paralyzer == false) then --needed to stop possible loops
			if uID and SpValidUnitID(uID) then
				...
				local health, maxHealth, paralyzeDamage, capture, build = Engine.Shared.GetUnitHealth(uID)
				Engine.Synced.AddUnitDamage(uID, maxHealth * 3, stunDuration, 99, weaponID) --no weapon ID, no stun. with weapon ID, infinite loops, even with the 99 exclusion. -1 does not work.
	...
	function gadget:GameFrame(frame)
		...
		if Engine.Shared.GetGameFrame() % 15 == 0 then
				...
				if expl.t >= curtime - effectlength then
					...
					for i = 1, #unitIDsBig do
						...
						if toStunUnits[unitDefID] then
							...
							if (dx * dx + dz * dz) > (q * (radius - width)) * (q * (radius - width)) then
								-- linear and not O(n^2)
								local health, maxHealth, paralyzeDamage, capture, build = Engine.Shared.GetUnitHealth(unitID)
								...
								if paralyzeDamage < maxHealth * 1.2 then --try to prevent excessive stun times, also needless restuns
									Engine.Synced.AddUnitDamage(unitID, maxHealth * 2, 5, 99, WeaponDefNames.corjuno_juno_pulse_ghost.id) ---...close enough?
									Engine.Synced.SpawnCEG("juno-damage", px, py + 8, pz, 0, 1, 0)
								end
