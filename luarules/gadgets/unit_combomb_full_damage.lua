...
function gadget:UnitPreDamaged(unitID, unitDefID, unitTeam, damage, paralyzer, weaponDefID, projectileID, attackerID, attackerDefID, attackerTeam) --we use UnitPreDamaged so as we get in before unit_transportfix has its effect
	...
	if weaponDefID == COM_BLAST and attackerID ~= nil and Engine.Shared.ValidUnitID(attackerID) then -- we control the damage inflicted on units by the COM_BLAST. Very rarely an invalid attackerID is returned with weaponID=COM_BLAST, I have no idea why/how.
		--Spring.Echo("weapon is comblast from unloaded com " .. attackerID)
		local x, y, z = Engine.Shared.GetUnitBasePosition(attackerID)
		local h = Engine.Shared.GetGroundHeight(x, z)
		--Spring.Echo(x .. " " .. y .. " " .. z .. " " .. h)
		if y - h > 10 then
			local newdamage = select(2, Engine.Shared.GetUnitHealth(unitID)) * 0.6
