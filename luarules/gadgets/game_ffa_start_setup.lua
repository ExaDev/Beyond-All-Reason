...
if not BAR.Utilities.Gametype.IsFFA() or not gadgetHandler:IsSyncedCode() then
	return
end
...
local function tryLoadConfigFromBAR(currentMapName)
	...
	for _, configFile in pairs(availableConfigFiles) do
		...
		if string.find(sanitizedCurrentMapName, basename) then
			Engine.Shared.Log(gadget:GetInfo().name, LOG.INFO, string.format("found FFA start points config %s provided by BAR for current map %s", configFile, currentMapName))
...
local function tryLoadConfigFromMap(currentMapName)
	...
	if VFS.FileExists(configFile) then
		Engine.Shared.Log(gadget:GetInfo().name, LOG.INFO, string.format("found FFA start points config %s provided by current map %s", configFile, currentMapName))
		return VFS.Include(configFile), true
	end
	return nil, false
end
...
local function tryLoadConfig(currentMapName, requiredStartPointCount)
	...
	if not found then
		Engine.Shared.Log(gadget:GetInfo().name, LOG.INFO, string.format("did not find a FFA start points config for current map %s", currentMapName))
		return nil
	end
	...
	if ffaStartPoints and ffaStartPoints[requiredStartPointCount] then
		Engine.Shared.Log(gadget:GetInfo().name, LOG.WARNING, string.format("backwards compatibility layer: using legacy FFA start points config for %s start points", requiredStartPointCount))
	...
	if config and config.startPoints and config.byAllyTeamCount and config.byAllyTeamCount[requiredStartPointCount] then
		Engine.Shared.Log(gadget:GetInfo().name, LOG.INFO, string.format("using FFA start points config for %s start points", requiredStartPointCount))
	...
	if not layout then
		Engine.Shared.Log(gadget:GetInfo().name, LOG.INFO, string.format("did not find a layout for %s start points for current map %s", requiredStartPointCount, currentMapName))
		return nil
	elseif #layout ~= requiredStartPointCount then
		Engine.Shared.Log(gadget:GetInfo().name, LOG.ERROR, string.format("incorrect number of start points found in layout (actual: %s, expected: %s)", #layout, requiredStartPointCount))
		Engine.Shared.Log(gadget:GetInfo().name, LOG.ERROR, "FFA start points config is malformed and will NOT be used")
		return nil
	else
		return layout
	end
...
local function setFFAStartPoints(allyTeamList)
	...
	if layout then
		...
		Engine.Shared.Log(gadget:GetInfo().name, LOG.INFO, "set up start points from FFA start points config for current map")
	end
end
...
local function shuffleStartBoxes(allyTeamList)
	...
	for _, allyTeamID in pairs(allyTeamList) do
		if startBoxes[allyTeamID] then
			...
			Engine.Synced.SetAllyTeamStartBox(allyTeamID, xmin, zmin, xmax, zmax)
		end
	end

	Engine.Shared.Log(gadget:GetInfo().name, LOG.INFO, "shuffled start boxes for ally teams (humans and AIs, but not Raptors and Scavengers)")
end
...
function gadget:Initialize()
	...
	if Engine.Shared.GetModOptions().teamffa_start_boxes_shuffle then
