...
if gadgetHandler:IsSyncedCode() then
	...
	local function SpawnUnitDefsForTeamSynced(teamID, unitDefName)
		...
		local unitcount = Engine.Shared.GetTeamUnitDefCount(teamID, unitDefID)
		...
		if unitcount < maxunits then
			...
			for x = 1, sqrtfeed do
				for z = 1, sqrtfeed do
					if numspawned < feedstep then
						...
						local unitID = Engine.Synced.CreateUnit(unitDefID, px, py, pz, "n", teamID)
						if unitID then
			...
			local gh = Engine.Shared.GetGroundHeight(ncx, ncz)
			Engine.Shared.GiveOrderToUnitArray(newUnitIDs, CMD.MOVE, { ncx, gh, ncz }, { "shift" })

			ncx = mapcx + placementradius * (getrandom() - 0.5)
			ncz = mapcz + placementradius * (getrandom() - 0.5)
			gh = Engine.Shared.GetGroundHeight(ncx, ncz)
			Engine.Shared.GiveOrderToUnitArray(newUnitIDs, CMD.MOVE, { ncx, gh, ncz }, { "shift" })

			gh = Engine.Shared.GetGroundHeight(cx, cz)
			Engine.Shared.GiveOrderToUnitArray(newUnitIDs, CMD.MOVE, { cx, gh, cz }, { "shift" })
	...
	function benchmark(words)
		...
		if not benchmarkenabled then
			Engine.Shared.Echo(string.format("Benchmark ended, %d units spawned over %d gameframes, Units/frame = %f", benchmarktotalunitsspawned, Engine.Shared.GetGameFrame() - benchmarkstartgameframe, benchmarktotalunitsspawned * (1.0 / (Engine.Shared.GetGameFrame() - benchmarkstartgameframe))))
			ExecuteRemoveUnitDefName(team1unitDefName)
			ExecuteRemoveUnitDefName(team2unitDefName)
			return
		end
		benchmarkstartgameframe = Engine.Shared.GetGameFrame()
		benchmarktotalunitsspawned = 0
		initrandom(7654321)
		if words[2] and UnitDefNames[words[2]] then
			team1unitDefName = words[2]
		else
			Engine.Shared.Echo(words[2], "is not a valid unitDefName, using", team1unitDefName, "instead")
		end

		if words[3] and UnitDefNames[words[3]] then
			team2unitDefName = words[3]
		else
			Engine.Shared.Echo(words[3], "is not a valid unitDefName, using", team2unitDefName, "instead")
		end

		if words[4] then
			local maxunitsint = tonumber(words[4])
			if maxunitsint == nil then
				Engine.Shared.Echo(words[4], "must be the number of max units to keep spawning, using", maxunits, "instead")
			else
		...
		if words[5] then
			...
			if feedstepint == nil then
				Engine.Shared.Echo(words[5], "must be the number units to spawn each step, using", maxunits, "instead")
			else
		...
		if words[6] then
			...
			if placementradiusint == nil then
				Engine.Shared.Echo(words[6], "must be the radius in which to spawn units, using ", placementradius, "instead")
			else
		...
		if words[7] then
			...
			if keepfeaturesint == nil then
				Engine.Shared.Echo(words[7], "must be the number of frames wrecks will live ", placementradius, "instead")
			else
				keepfeatures = math.floor(keepfeaturesint)
			end
		end

		Engine.Shared.Echo(string.format("Starting benchmark %s vs %s with %i maxunits and %i units per step in a %d radius, features live %d frames", team1unitDefName, team2unitDefName, maxunits, feedstep, placementradius, keepfeatures))
		featuredefstoremove = {}
		for _, udn in ipairs({ team1unitDefName, team2unitDefName }) do
			for _, wreckheap in ipairs({ "_dead", "_heap" }) do
				if FeatureDefNames[udn .. wreckheap] and FeatureDefNames[udn .. wreckheap].id then
					...
					Engine.Shared.Echo(udn .. wreckheap)
				end
			end
		end
	end
	...
	function gadget:FeatureCreated(featureID, allyTeam)
		if benchmarkenabled then
			...
			if featureDefID and featuredefstoremove[featureDefID] then
				featurestoremove[featureID] = Engine.Shared.GetGameFrame() + keepfeatures
			end
		end
	end
	...
	function benchmark(_, line, words, playerID, action)
		if playerID ~= Engine.Unsynced.GetLocalPlayerID() then
			return
		end
		Engine.Shared.Echo("Benchmark", line, words, playerID, action)
		if not isAuthorized(playerID, "terrain") then
			return
		end
		if benchmarkactive then
			...
			Engine.Shared.Echo("Starting Benchmark")
