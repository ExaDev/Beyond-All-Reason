...
function MapHST:Init()
	local rules_param_mex_count = Engine.Shared.GetGameRulesParam("mex_count")
...
function MapHST:InitPathCost()
	...
	self:EchoDebug("INIT new array node cost", Engine.Shared.InitPathNodeCostsArray(game:GetTeamID(), self.heightMapX, self.heightMapZ))
	local cost = 0
	local index = 1
	...
	for i = 1, self.heightMapX do
		for j = 1, self.heightMapZ do
			...
			Engine.Shared.SetPathNodeCost(game:GetTeamID(), index, cost)
			--self:EchoDebug('init path node cost',i,j,Spring.GetPathNodeCost(id,i,j))
			cost = 0
			index = index + 1
		end
	end
	self:EchoDebug("SET new array node layer non sync", Engine.Shared.SetPathNodeCosts(game:GetTeamID()))
...
function MapHST:NewCell(gx, gz)
	...
	cellPos.y = Engine.Shared.GetGroundHeight(x, z)
...
function MapHST:moveLayerTest(pos) --check where units can stay or not
	...
	for layer, unitName in pairs(self.ai.armyhst.mobUnitExampleName) do
		...
		if Engine.Shared.TestMoveOrder(self.ai.armyhst.unitTable[unitName].defId, pos.x, pos.y, pos.z, nil, nil, nil, true, false, true) then
			layers[layer] = 0
		else
			layers[layer] = false
		end
			...
			for layer, net in pairs(self.networks) do
				if layer ~= "air" then -- air list is already filled
					for index, network in pairs(layer) do
						if self.startLocations[layer] == nil then
							self.startLocations[layer] = {}
						end
						if self.startLocations[layer][index] == nil then
							self.startLocations[layer][index] = {}
						end
...
function MapHST:MobilityNetworkHere(mtype, position)
	if not mtype or not position then
		return nil
	end
	...
	for cx = 1, self.gridSideX, cellsPerNodeSide do
		...
		for cz = 1, self.gridSideZ, cellsPerNodeSide do
			...
			if goodCellsCount > 0 then
				...
				position.y = Engine.Shared.GetGroundHeight(x, z)
