...
function EcoHST:Init()
	...
	local McurrentLevel, Mstorage, Mpull, Mincome, Mexpense, Mshare, Msent, Mreceived = Engine.Shared.GetTeamResources(self.ai.id, "metal")
	local EcurrentLevel, Estorage, Epull, Eincome, Eexpense, Eshare, Esent, Ereceived = Engine.Shared.GetTeamResources(self.ai.id, "energy")
	for i = 1, average do
...
function EcoHST:Update()
	...
	local currentLevel, storage, pull, income, expense, share, sent, received = Engine.Shared.GetTeamResources(self.ai.id, "metal")
	...
	currentLevel, storage, pull, income, expense, share, sent, received = Engine.Shared.GetTeamResources(self.ai.id, "energy")
