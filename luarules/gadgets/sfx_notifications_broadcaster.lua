...
if not gadgetHandler:IsSyncedCode() then
	...
	function BroadcastEvent(_, event, player, forceplay)
		if Script.LuaUI("NotificationEvent") and (forceplay or (tonumber(player) and ((tonumber(player) == Engine.Unsynced.GetLocalPlayerID()) or Engine.Unsynced.GetSpectatingState()))) then
			if forceplay then
				forceplay = " y"
			else
				forceplay = ""
