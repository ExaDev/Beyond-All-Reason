...
if BAR.Utilities.Gametype.IsRaptors() then
	Engine.Shared.Log(gadget:GetInfo().name, LOG.INFO, "Raptor Defense Spawner Activated!")
elseif BAR.Utilities.Gametype.IsScavengers() then
	...
	Engine.Shared.Log(gadget:GetInfo().name, LOG.INFO, "Defense Spawner Deactivated!")
	return false
end
	...
	if frame > lastTurretFrame + 150 then
		if frame % 30 == 9 then
			for unitID, data in pairs(aliveBuilders) do
				if (Engine.Shared.GetUnitNearestEnemy(unitID, data.range * 5, true) and math.random(0, 15) == 0) or data.isFactory then
					--Spring.Echo(data.unitDefName, "NearestEnemyInRange")
					local unitCommands = not data.isFactory and Engine.Shared.GetUnitCommands(unitID, 1) or {}
					if (data.isFactory and #Engine.Shared.GetFullBuildQueue(unitID) < 5) or (not data.isFactory and (not unitCommands[1] or (unitCommands[1] and unitCommands[1].id > 0 and unitCommands[1].id ~= CMD.REPAIR))) then
						...
						if #turretOptions > 1 then
							...
							Engine.Shared.GiveOrderToUnit(unitID, -turret, { x + math.random(-data.range, data.range), y, z + math.random(-data.range, data.range) }, {})
