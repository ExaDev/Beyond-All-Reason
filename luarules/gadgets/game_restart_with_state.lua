	...
	function gadget:RecvLuaMsg(msg, playerID)
		...
		if chunk then
			if restoreBuffer then
				restoreBuffer[#restoreBuffer + 1] = chunk
			end
			return true
		end
		...
		for i = 1, #data.units do
			...
			if newID and u.commands and #u.commands > 0 then
				...
				for j = 1, #u.commands do
					...
					-- skip
						...
						local skip = false
						local isUnitTarget = UNIT_TARGET_ALWAYS[c.id] or (UNIT_TARGET_SINGLE[c.id] and params and #params == 1)
			...
			if newID and u.factoryCommands and #u.factoryCommands > 0 then
				for j = 1, #u.factoryCommands do
					...
					local function clearBit(v, bit)
						if (math.floor(v / bit) % 2) == 1 then
							return v - bit
						end
						return v
					end
