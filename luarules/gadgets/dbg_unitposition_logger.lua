...
if not gadgetHandler:IsSyncedCode() then
	...
	local DEBUG = select(1, Engine.Shared.GetPlayerInfo(Engine.Unsynced.GetLocalPlayerID())) == "[teh]Flow"
		...
		if log[frame] then
			...
			if noParts then
				...
				if DEBUG and (attempts ~= "1" or part > tonumber(numParts)) then
					Engine.Shared.Echo('UNITLOG: "all received": frame:' .. frame .. " part:" .. part .. " parts:" .. numParts .. " attempts:" .. attempts)
				end
			end
		end
	end
	...
	local function sendLog(frame, part, attempts)
		Engine.Unsynced.SendLuaRulesMsg("log" .. validation .. frame .. ";" .. part .. ";" .. #log[frame].participants .. ";" .. attempts .. ";" .. VFS.ZlibCompress(Json.encode(log[frame].parts[part])))
	end
	...
	function gadget:GameFrame(gf)
			...
			if log[frame] then
				...
				if log[frame].attempts > #log[frame].participants then -- this should not happen... if so, something went wrong because we tried resending by all other participants already
					if DEBUG then
						...
						Engine.Shared.Echo('UNITLOG: "max attempts reached": frame:' .. frame .. " attempts:" .. log[frame].attempts .. "of " .. #log[frame].participants .. " missing parts:" .. missingParts .. " missing units:" .. missingUnits)
					end
					log[frame] = nil
				else
					-- loop leftover parts
		...
		if gf >= lastLogFrame + logRate then
			...
			for frame, params in pairs(log) do
				if frame < gf - maxLogMemoryDuration then
					if DEBUG then
						...
						Engine.Shared.Echo('UNITLOG: "timeout": frame:' .. frame .. " attempts:" .. log[frame].attempts .. "of " .. #log[frame].participants .. " missing parts:" .. missingParts .. " missing units:" .. missingUnits)
					end
					log[frame] = nil
				end
			end
			...
			for _, playerID in ipairs(Engine.Shared.GetPlayerList()) do
				local name, _, _, teamID, _, ping = Engine.Shared.GetPlayerInfo(playerID, false)
				...
				local isDead = select(4, Engine.Shared.GetTeamInfo(teamID))
				if (ping > 0.01 or isSinglePlayer) and ping < pingCutoff / 1000 and not Engine.Shared.GetTeamLuaAI(teamID) and not isDead then
