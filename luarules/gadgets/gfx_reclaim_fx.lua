...
if gadgetHandler:IsSyncedCode() then
	...
	function gadget:UnitDestroyed(unitID, unitDefID, unitTeam, attackerID, attackerDefID, attackerTeam, weaponDefID)
		if attackerID == nil and not ignoreUnits[unitDefID] then -- if reclaimed
			...
			if ux ~= nil then
				...
				Engine.Synced.SpawnCEG("metalshards1", x, y, z)
				...
				for i = 1, unitNumFx[unitDefID], 1 do
					...
					Engine.Synced.SpawnCEG("metalshards" .. (((i + 1) % 3) + 1), x, y, z)
				end
			end
		end
	end
	...
	function gadget:FeatureDamaged(featureID, featureDefID, featureTeam, damage, weaponDefID, projectileID, attackerID, attackerDefID, attackerTeam)
		if damage > 4 and (damagedFeatures[featureID] == nil or Engine.Shared.GetGameFrame() - damagedFeatures[featureID] > 15) and select(1, Engine.Shared.GetFeatureHealth(featureID)) > 0 then -- not sure if "select(1,Spring.GetFeatureHealth(featureID)) > 0" is needed because if featuredestroyed, featuredamaged wont be called probably
			...
			if fx ~= nil then
				...
				if fdef and fdef.minx and fdef.maxx then
					...
					local _, mm, _, me, _ = Engine.Shared.GetFeatureResources(featureID)
					if me ~= nil and me > 0 then
						local numFx = math.min(25, math.floor(me / 250))
						local posMultiplier = 0.5
						Engine.Synced.SpawnCEG("energyshards1", x, y, z)
						for i = 1, numFx, 1 do
							...
							Engine.Synced.SpawnCEG("energyshards" .. (((i + 1) % 3) + 1), x, y, z)
						end
					end
			...
			damagedFeatures[featureID] = Engine.Shared.GetGameFrame()
		end
	end
	...
	function gadget:FeatureDestroyed(featureID, allyteam)
		...
		local fx, fy, fz = Engine.Shared.GetFeaturePosition(featureID)
		if fy ~= nil and fy + 15 > Engine.Shared.GetGroundHeight(fx, fz) then
			local fdef = featureList[Engine.Shared.GetFeatureDefID(featureID)]
			if fdef and fdef.minx then
				local x, y, z = fx, fy, fz
				local rm, mm, re, me, rl = Engine.Shared.GetFeatureResources(featureID)
				if me ~= nil and me > 0 then
	...
	local function reclaimfx(_, fx, fy, fz)
		...
		if fullview or Engine.Shared.IsPosInLos(fx, fy, fz, Engine.Unsynced.GetLocalAllyTeamID()) then
