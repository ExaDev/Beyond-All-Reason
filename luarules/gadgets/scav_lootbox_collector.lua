...
if (not gadgetHandler:IsSyncedCode()) or (not BAR.Utilities.Gametype.IsScavengers()) or Engine.Shared.GetModOptions().unit_restrictions_noair then
	return false
...
local RaptorStartboxXMin, RaptorStartboxZMin, RaptorStartboxXMax, RaptorStartboxZMax = Engine.Shared.GetAllyTeamStartBox(scavAllyTeamID)
...
function gadget:GameFrame(frame)
	if frame % 30 == 12 and Engine.Shared.GetGameRulesParam("scavBossAnger") >= 1 and Engine.Shared.GetGameRulesParam("scavTechAnger") >= config.airStartAnger then
		if aliveLootboxesCount > 0 and aliveSpawnersCount > 0 then
			...
			if frame - math.ceil(18000 / aliveLootboxesCount) > lastTransportSentFrame then -- 10 minutes for 1 lootbox alive
				...
				for lootboxID, lootboxTier in pairs(aliveLootboxes) do
					local lootboxPosX, lootboxPosY, lootboxPosZ = Engine.Shared.GetUnitPosition(lootboxID)
								...
								if math.random(0, SetCount(transportsList)) == 0 and transportTier == lootboxTier and not handledLootboxesList[lootboxID] then
									for spawnerID, _ in pairs(aliveSpawners) do
										if math.random(0, SetCount(aliveSpawners)) == 0 and not handledLootboxesList[lootboxID] then
											...
											local spawnerPosX, spawnerPosY, spawnerPosZ = Engine.Shared.GetUnitPosition(spawnerID)
											for j = 1, 5 do
												if math.random() <= config.spawnChance then
													local transportID = Engine.Synced.CreateUnit(transportDefID, spawnerPosX + math.random(-1024, 1024), spawnerPosY + 100, spawnerPosZ + math.random(-1024, 1024), math.random(0, 3), scavTeamID)
													if transportID then
														handledLootboxesList[targetLootboxID] = true
														success = true
														lastTransportSentFrame = frame
														Engine.Shared.GiveOrderToUnit(transportID, CMD.LOAD_UNITS, { targetLootboxID }, { "shift" })
														for i = 1, 100 do
															local randomX = math.random(0, Game.mapSizeX)
															local randomZ = math.random(0, Game.mapSizeZ)
															local randomY = math.max(0, Engine.Shared.GetGroundHeight(randomX, randomZ))
															if GG.IsPosInRaptorScum(randomX, randomY, randomZ) then
																Engine.Shared.GiveOrderToUnit(transportID, CMD.UNLOAD_UNITS, { randomX, randomY, randomZ, 1024 }, { "shift" })
															end
															if i == 100 then
																Engine.Shared.GiveOrderToUnit(transportID, CMD.MOVE, { randomX + math.random(-256, 256), randomY, randomZ + math.random(-256, 256) }, { "shift" })
															end
														end
													end
												end
