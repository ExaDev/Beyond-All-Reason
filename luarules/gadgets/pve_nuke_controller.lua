...
if BAR.Utilities.Gametype.IsRaptors() then
	...
	difficulty = Engine.Shared.GetModOptions().raptor_difficulty
elseif BAR.Utilities.Gametype.IsScavengers() then
	...
	difficulty = Engine.Shared.GetModOptions().scav_difficulty
else
...
function gadget:UnitCreated(unitID, unitDefID, unitTeam)
	if nukeDefs[unitDefID] and (unitTeam == pveTeamID) then
		aliveNukeLaunchers[unitID] = Engine.Shared.GetGameSeconds() + math.random(5, 10)
	end
end
	...
	if frame % 30 == 17 then
		...
		for nukeID, cooldown in pairs(aliveNukeLaunchers) do
			if cooldown <= GetGameSeconds() then
				...
				if Engine.Shared.GetUnitTeam(targetID) ~= Engine.Shared.GetUnitTeam(nukeID) then
					local x, y, z = Engine.Shared.GetUnitPosition(targetID)
					x = x + math.random(-1024, 1024)
					z = z + math.random(-1024, 1024)
					y = math.max(Engine.Shared.GetGroundHeight(x, z), 0)
					if x and z and x > 0 and x < mapSizeX and z > 0 and z < mapSizeZ and checkTargetCell(x, z, nukeID) then
						Engine.Shared.GiveOrderToUnit(nukeID, CMD.ATTACK, { x, y, z }, 0)
