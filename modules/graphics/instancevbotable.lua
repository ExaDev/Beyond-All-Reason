...
local function validateInstanceVBOTable(iT, calledfrom)
	...
	for i = 1, iT.usedElements do
		if iT.indextoInstanceID[i] == nil then
			Engine.Shared.Echo("There is a hole in indextoInstanceID", iT.myName, "at", i, "out of", iT.usedElements, calledfrom)
			--Spring.Echo()
			if iT.indextoUnitID[i] == nil then
				Engine.Shared.Echo("It is also missing from indextoUnitID")
			else
				Engine.Shared.Echo("But it does exist in indextoUnitID with an unitID of ", iT.indextoUnitID[i])
				Engine.Shared.Echo("This is valid?", Engine.Shared.GetUnitPosition(iT.indextoUnitID[i]))
			end
		else
			local instanceID = iT.indextoInstanceID[i]
			if iT.instanceIDtoIndex[instanceID] == nil then
				Engine.Shared.Echo("There is a hole instanceIDtoIndex", iT.myName, "at", i, " iT.instanceIDtoIndex[instanceID] == nil ")
			elseif iT.instanceIDtoIndex[instanceID] ~= i then
				Engine.Shared.Echo("There is a problem in indextoInstanceID", iT.myName, "at i =", i, "  iT.indextoInstanceID[instanceID] ~= i, it is instead: ", iT.indextoInstanceID[instanceID])
			end
		end
	end
	...
	if (indextoInstanceIDsize ~= instanceIDtoIndexsize) or (instanceIDtoIndexsize ~= indextoUnitID) then
		Engine.Shared.Echo("Table size mismatch during validation of", iT.myName, indextoInstanceIDsize, instanceIDtoIndexsize, indextoUnitID)
	end
end
...
function locateInvalidUnits(iT)
	...
	for i, unitID in ipairs(iT.indextoUnitID) do
		if iT.featureIDs then
			if Engine.Shared.ValidFeatureID(unitID) then
				local px, py, pz = Engine.Shared.GetFeaturePosition(unitID)
				local fdefname = FeatureDefs[Engine.Shared.GetFeatureDefID(unitID)].name
				iT.validinfo[unitID] = { px = px, py = py, pz = pz, fdefname = fdefname }
			else
				Engine.Unsynced.SendCommands({ "pause 1" })
				Engine.Shared.Echo("INVALID feature, last seen at", unitID)
				local vi = iT.validinfo[unitID]
				local markertext = tostring(unitID) .. "," .. dbgt(vi)
				Engine.Unsynced.MarkerAddPoint(vi.px, vi.py, vi.pz, markertext)
			...
			if Engine.Shared.ValidUnitID(unitID) then
				...
				Engine.Unsynced.SendCommands({ "pause 1" })
				Engine.Shared.Echo(iT.myName, " INVALID unitID", unitID, "#elements", iT.usedElements, "last seen at tablepos:", i)
...
local function resizeInstanceVBOTable(iT)
	...
	if iT.indextoUnitID then
		...
		for i, objectID in ipairs(iT.indextoUnitID) do
			...
			if iT.featureIDs then
				isValidID = Engine.Shared.ValidFeatureID(objectID)
			else
				isValidID = Engine.Shared.ValidUnitID(objectID)
			end
			if isValidID then
				...
				Engine.Shared.Echo("Warning: Found invalid unit/featureID", objectID, "at", i, "while resizing", iT.myName)
			end
		end
...
local function pushElementInstance(iT, thisInstance, instanceID, updateExisting, noUpload, unitID)
	...
	if unitID ~= nil then
		local isvalidid
		if iT.featureIDs then
			isvalidid = Engine.Shared.ValidFeatureID(unitID)
		else
			isvalidid = Engine.Shared.ValidUnitID(unitID)
		end
		if isvalidid == false then
			Engine.Shared.Echo("Error: Attempted to push an invalid unit/featureID", unitID, "into", iT.myName)
			noUpload = true
...
local function popElementInstance(iT, instanceID, noUpload)
	...
	if instanceID == nil then
		Engine.Shared.Echo("Tried to remove element with nil instanceID from instanceTable " .. iT.myName)
		return nil
	end
	...
	if oldElementIndex == lastElementIndex then
		...
		if lastElementInstanceID == nil then --
			Engine.Shared.Echo("We somehow have a nil element at the back of the array, which is completely invalid, probably about to crash", iT.myName)
		...
		if iT.indextoUnitID then
			...
			if popunitID == nil then
				Engine.Shared.Echo("TODO: what the f is happening here?, how the f could we have popped a nil from the back of?", iT.myName) -- TODO TODO
			end
					...
					if iT.lastpopgameframe ~= gf then -- New gameframe
						...
						if iT.numZombies and iT.numZombies > 0 then -- WE HAVE ZOMBIES AAAAARGH
							...
							for zombie, gf in pairs(iT.zombies) do
								...
								Engine.Shared.Echo("ZOMBIE instanceID", zombie, "gf", gf)
			...
			if (iT.featureIDs and Engine.Shared.ValidFeatureID(popunitID)) or Engine.Shared.ValidUnitID(popunitID) then
				if noUpload ~= true then
					if iT.featureIDs then
...
local function getElementInstanceData(iT, instanceID, cacheTable)
	...
	if instanceIndex == nil then
		Engine.Shared.Echo("Tried to getElementInstanceData from", iT.myName, instanceID, "but it does not exist")
		return nil
	end
...
local function countInvalidUnitIDs(iT)
	...
	if #invalids > 0 then
		Engine.Shared.Echo(#invalids, "invalid IDs found in ", iT.myName)
	end
	return invalids
end
