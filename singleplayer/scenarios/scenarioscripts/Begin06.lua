...
local function Fail()
	...
	GameOver = Engine.Synced.GameOver({ 2 }) --winningAllyTeamN where N is Ally ID
end
...
local function Loadout()
	for k, unit in pairs(loadout) do
		if UnitDefNames[unit.name] then
			...
			local unitID = Engine.Synced.CreateUnit(unit.name, unit.x, unit.y, unit.z, unit.rot, unit.teamID)
			for i = 1, #unit.queue do
				local order = unit.queue[i]
				local position = { order.position.px, order.position.py, order.position.pz }
				Engine.Shared.GiveOrderToUnit(unitID, order.cmdID, position, CMD.OPT_SHIFT)
			end
		end
	end
end
